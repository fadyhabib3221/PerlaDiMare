...
import ExtractedActivitiesSection from "./components/ActivitiesSection";
import ExtractedTimeSelect from "./components/TimeSelect";
import ExtractedToggleSwitch from "./components/ToggleSwitch";
...
// writes cell styles (fills/fonts) into the .xlsx file â€” needed for the export's
// alternating row shading and the highlighted totals row. Plain "xlsx" silently
// drops any style info on write.
import * as XLSX from "xlsx-js-style";
// Ù…Ø¬Ø§Ù†ÙŠ Ø¨Ø§Ù„ÙƒØ§Ù…Ù„ ÙˆÙ…Ø­Ù„ÙŠ ÙÙŠ Ø§Ù„Ù…ØªØµÙØ­ â€” Ø¨Ø¯ÙŠÙ„ Ø¹Ù† Ø§Ø³ØªØ¯Ø¹Ø§Ø¡ Claude Vision API Ù„Ù‚Ø±Ø§Ø¡Ø© ØµÙˆØ± Ø§Ù„ØªØ°Ø§ÙƒØ±.
// Ø§Ù„Ø¯Ù‚Ø© Ø£Ø¶Ø¹Ù Ù…Ù† Ø§Ù„Ù€ AI vision Ù„Ø£Ù†Ù‡ OCR ØªÙ‚Ù„ÙŠØ¯ÙŠ (Ø¨ÙŠÙ‚Ø±Ø§ Ø§Ù„Ø­Ø±ÙˆÙ Ø¨Ø³ØŒ Ù…Ø´ Ø¨ÙŠÙÙ‡Ù… Ø´ÙƒÙ„ Ø§Ù„ØªØ°ÙƒØ±Ø©)ØŒ
// ÙÙ…Ø­ØªØ§Ø¬ Ø¨Ø¹Ø¯Ù‡ regex Ø¹Ø´Ø§Ù† Ù†Ø­Ø§ÙˆÙ„ Ù†Ù„Ù‚Ø· Ø§Ù„Ø­Ù‚ÙˆÙ„ (Ø§Ø³Ù… Ø§Ù„Ø±Ø§ÙƒØ¨ØŒ Ø±Ù‚Ù… Ø§Ù„ØªØ°ÙƒØ±Ø©ØŒ PNRØŒ Ø§Ù„Ù…Ø·Ø§Ø±Ø§ØªØŒ Ø§Ù„ØªØ§Ø±ÙŠØ®).
import { createWorker } from "tesseract.js";
// Ù…ÙƒØªØ¨Ø© ØªÙˆÙ„ÙŠØ¯ QR Code Ù…Ø­Ù„ÙŠÙ‹Ø§ ÙÙŠ Ø§Ù„Ù…ØªØµÙØ­ (Ø¨Ø¯ÙˆÙ† Ø£ÙŠ Ø§ØªØµØ§Ù„ Ø¨Ø£ÙŠ Ø³ÙŠØ±ÙØ± Ø®Ø§Ø±Ø¬ÙŠ) â€” Ø¨Ù†Ø³ØªØ®Ø¯Ù…Ù‡Ø§
// Ø¹Ø´Ø§Ù† Ù†Ø±Ø³Ù… ÙƒÙˆØ¯ Ø§Ù„Ù€ QR Ø§Ù„Ø®Ø§Øµ Ø¨Ø¥Ø¹Ø¯Ø§Ø¯ Ø§Ù„Ù€ 2FA ÙƒØµÙˆØ±Ø© data-URL Ø¨Ø¯Ù„ Ù…Ø§ Ù†Ø¹Ø±Ø¶ Ø§Ù„Ù…ÙØªØ§Ø­
// ÙƒÙ†Øµ Ø·ÙˆÙŠÙ„ Ø¨Ø³.
...
// frame; anywhere else (a raw preview, a downloaded copy opened in another tool, etc.) it's
// simply undefined, and every call below like `window.storage.get(...)` throws synchronously
// with "Cannot read properties of undefined (reading 'get')" â€” crashing the app on first
// mount before the error boundary even has real data to show. This shim installs an
// in-memory stand-in with the exact same contract (including: a missing key REJECTS instead
// of resolving to null, matching the real API) whenever the host hasn't provided one, so the
// app always renders. Data just won't persist across reloads in that fallback case.
if (typeof window !== "undefined" && !window.storage) {
  const __mem = new Map();
  const __k = (key, shared) => `${shared ? "shared" : "personal"}:${key}`;
  window.storage = {
    async get(key, shared = false) {
      const k = __k(key, shared);
      if (!__mem.has(k)) throw new Error(`Key not found: ${key}`);
      return { key, value: __mem.get(k), shared };
    },
    async set(key, value, shared = false) {
      __mem.set(__k(key, shared), value);
      return { key, value, shared };
    },
    async delete(key, shared = false) {
      const k = __k(key, shared);
      const deleted = __mem.delete(k);
      return { key, deleted, shared };
    },
    async list(prefix = "", shared = false) {
      const suffix = shared ? "shared:" : "personal:";
      const keys = [...__mem.keys()]
        .filter((k) => k.startsWith(suffix) && k.slice(suffix.length).startsWith(prefix))
        .map((k) => k.slice(suffix.length));
      return { keys, prefix, shared };
    },
  };
}

// Travel Agency Management logo, embedded as a data URI so the app works as a single self-contained file.
...
import {
  ...
  BarChart3, GripVertical, Unlock, Monitor, CalendarOff, Ban, Smartphone,
} from "lucide-react";
...
const VISA_COUNTRIES = [
  ...
  { code: "TR", name: "TÃ¼rkiye" }, { code: "IR", name: "Iran" }, { code: "PK", name: "Pakistan" },
...
// not a bare unsalted hash â€” a random per-password salt means two employees who pick
...
// API â€” no server involved. Stored format: "pbkdf2:<iterations>:<saltHex>:<hashHex>".
const PBKDF2_PREFIX = "pbkdf2:";
const PBKDF2_ITERATIONS = 100000;
const LEGACY_SHA256_PREFIX = "sha256:"; // old unsalted format â€” verifyPassword still reads it so pre-existing accounts aren't locked out; needsRehash() flags it for silent upgrade
...
// Time-based one-time codes generated by a standard authenticator app (Google
// Authenticator, Microsoft Authenticator, Authy, etc.) â€” nothing is emailed or texted
// anywhere; the shared secret lives only in the employee record and in the app on
// their phone, and both sides independently derive the same 6-digit code from the
// current time. Verification runs entirely client-side via the browser's Web Crypto
// HMAC-SHA1 implementation â€” no server or third-party service involved.
...
// Generates a fresh random 160-bit secret â€” the standard size for TOTP â€” encoded as
// Base32 (the format authenticator apps expect for manual key entry).
const generateTotpSecret = () => base32Encode(crypto.getRandomValues(new Uint8Array(20)));

// otpauth:// URI some authenticator apps can import directly (via a QR scanner or a
// "paste a link" option); the same secret is also shown as plain text for apps that
// only support typing the key in by hand.
const buildTotpUri = (secretBase32, accountLabel, issuer = "Travel Agency Management") =>
...
// Computes the 6-digit code for one specific 30-second time-step counter (RFC 4226/6238).
...
// Verifies a code the employee just typed against the current time step, allowing one
// step of drift either way (Â±30s) so a slightly-off phone clock still works.
const verifyTotpCode = async (secretBase32, codeAttempt) => {
...
// itself is never stored anywhere in the clear â€” instead, every employee record
...
// Generates a brand-new random workspace key. Happens exactly once per workspace â€”
...
// Deliberately sessionStorage rather than localStorage â€” it's cleared when the tab/
// browser closes â€” and it's wiped on explicit logout. This is a conscious trade-off:
...
// refresh â€” if it's set, the app comes back up locked instead of silently showing
...
const saveLocalSession = async (user, workspaceKeyObj, startedAt) => {
  try {
    ...
    // Best-effort only â€” worst case, the employee just has to log in again after a refresh.
  }
};
...
// doing a cheap window.storage.get() + JSON.parse() â€” no decryption, no workspace key
// needed â€” instead of paying for a full decrypt just to answer "did this change?".
...
// it's re-saved encrypted right away â€” a one-time, silent migration.
...
const secureLoad = async (storageKey, workspaceKey, fallback, options = {}) => {
  ...
  if (parsed && parsed.__enc === ENC_MARKER) {
    if (!workspaceKey) return asPlain(fallback); // locked â€” no key available yet
...
// order the underlying array happens to be in â€” same idea as the Flights table's
...
// Same as formatDisplayDate but spells the month out in full (e.g. "06-AUGUST-2026") â€”
...
// Reduces an employee's full name to just the first letter of each word â€” e.g.
// "Fady Habib" -> "FH" â€” used in the main flight-tickets table so the Employee
...
//
// IMPORTANT â€” read before editing this list:
// This is a client-side app, so this gate can never be a hard security boundary â€”
...
const checkLicenseCode = async (rawCode) => {
  ...
  // Return the normalized input code itself (not a secret â€” the person who typed
...
// "type" marks whether this passenger is an Adult, Child, or Infant â€” Child/Infant
...
// so every customer defaults to "adult" â€” this keeps old tickets computing exactly as
// they always have.
...
// emdAmount (the EMD charged on the exchange) is added on top of the net side only â€”
...
// Default Flights supplier / booking source options â€” seeded into suggestions.flightSuppliers
...
// are delivered as a raw <script src="..."> snippet from the Travelpayouts dashboard â€” a
...
// ============================================================
// Expense categories offered on the expense form â€” covers the recurring cost lines a
// travel agency typically tracks. "Other" always stays free-text via a separate field.
const EXPENSE_CATEGORIES = [
  "Ø¥ÙŠØ¬Ø§Ø±",
  "Ù…Ø±ØªØ¨Ø§Øª ÙˆØ¹Ù…ÙˆÙ„Ø§Øª",
  "ÙƒÙ‡Ø±Ø¨Ø§Ø¡ ÙˆÙ…ÙŠØ§Ù‡",
  "Ø§ØªØµØ§Ù„Ø§Øª ÙˆØ¥Ù†ØªØ±Ù†Øª",
  "ØªØ³ÙˆÙŠÙ‚ ÙˆØ¥Ø¹Ù„Ø§Ù†",
  "ØµÙŠØ§Ù†Ø©",
  "Ø¶Ø±Ø§Ø¦Ø¨ ÙˆØ±Ø³ÙˆÙ… Ø­ÙƒÙˆÙ…ÙŠØ©",
  "Ø¨Ù†Ùƒ ÙˆØªØ­ÙˆÙŠÙ„Ø§Øª",
  "Ù‚Ø±Ø·Ø§Ø³ÙŠØ© ÙˆÙ…Ø·Ø¨ÙˆØ¹Ø§Øª",
  "Ø£Ø®Ø±Ù‰",
];
const TREASURY_ACCOUNT_TYPES = [
  { value: "cash", label: "Ø®Ø²ÙŠÙ†Ø© Ù†Ù‚Ø¯ÙŠØ©" },
  { value: "bank", label: "Ø­Ø³Ø§Ø¨ Ø¨Ù†ÙƒÙŠ" },
];
// Categories for manual treasury entries (money moved without a linked customer/
// supplier/expense record â€” capital injections, owner drawings, transfers, etc.).
const TREASURY_ENTRY_CATEGORIES_IN = ["Ø±Ø£Ø³ Ù…Ø§Ù„ / Ø¥ÙŠØ¯Ø§Ø¹ Ù…Ø§Ù„Ùƒ", "ØªØ­ÙˆÙŠÙ„ Ù…Ù† Ø­Ø³Ø§Ø¨ Ø¢Ø®Ø±", "Ø¥ÙŠØ±Ø§Ø¯Ø§Øª Ø£Ø®Ø±Ù‰"];
const TREASURY_ENTRY_CATEGORIES_OUT = ["Ù…Ø³Ø­ÙˆØ¨Ø§Øª Ù…Ø§Ù„Ùƒ", "ØªØ­ÙˆÙŠÙ„ Ø¥Ù„Ù‰ Ø­Ø³Ø§Ø¨ Ø¢Ø®Ø±", "Ù…ØµØ±ÙˆÙØ§Øª Ø£Ø®Ø±Ù‰"];
...
const EXPENSE_CATEGORY_LABELS_EN = {
  "Ø¥ÙŠØ¬Ø§Ø±": "Rent",
  "Ù…Ø±ØªØ¨Ø§Øª ÙˆØ¹Ù…ÙˆÙ„Ø§Øª": "Salaries & Commissions",
  "ÙƒÙ‡Ø±Ø¨Ø§Ø¡ ÙˆÙ…ÙŠØ§Ù‡": "Electricity & Water",
  "Ø§ØªØµØ§Ù„Ø§Øª ÙˆØ¥Ù†ØªØ±Ù†Øª": "Communications & Internet",
  "ØªØ³ÙˆÙŠÙ‚ ÙˆØ¥Ø¹Ù„Ø§Ù†": "Marketing & Advertising",
  "ØµÙŠØ§Ù†Ø©": "Maintenance",
  "Ø¶Ø±Ø§Ø¦Ø¨ ÙˆØ±Ø³ÙˆÙ… Ø­ÙƒÙˆÙ…ÙŠØ©": "Taxes & Government Fees",
  "Ø¨Ù†Ùƒ ÙˆØªØ­ÙˆÙŠÙ„Ø§Øª": "Bank & Transfers",
  "Ù‚Ø±Ø·Ø§Ø³ÙŠØ© ÙˆÙ…Ø·Ø¨ÙˆØ¹Ø§Øª": "Stationery & Printing",
  "Ø£Ø®Ø±Ù‰": "Other",
};
const TREASURY_ACCOUNT_TYPE_LABELS_EN = { cash: "Cash Treasury", bank: "Bank Account" };
const TREASURY_ENTRY_CATEGORY_LABELS_EN = {
  "Ø±Ø£Ø³ Ù…Ø§Ù„ / Ø¥ÙŠØ¯Ø§Ø¹ Ù…Ø§Ù„Ùƒ": "Capital / Owner Deposit",
  "ØªØ­ÙˆÙŠÙ„ Ù…Ù† Ø­Ø³Ø§Ø¨ Ø¢Ø®Ø±": "Transfer From Another Account",
  "Ø¥ÙŠØ±Ø§Ø¯Ø§Øª Ø£Ø®Ø±Ù‰": "Other Income",
  "Ù…Ø³Ø­ÙˆØ¨Ø§Øª Ù…Ø§Ù„Ùƒ": "Owner Withdrawals",
  "ØªØ­ÙˆÙŠÙ„ Ø¥Ù„Ù‰ Ø­Ø³Ø§Ø¨ Ø¢Ø®Ø±": "Transfer To Another Account",
  "Ù…ØµØ±ÙˆÙØ§Øª Ø£Ø®Ø±Ù‰": "Other Expenses",
};
const ACCOUNTS_I18N = {
  ar: {
    tabOverview: "Ù†Ø¸Ø±Ø© Ø¹Ø§Ù…Ø©", tabSuppliers: "Ø§Ù„Ù…ÙˆØ±Ø¯ÙŠÙ†", tabCustomers: "Ø§Ù„Ø¹Ù…Ù„Ø§Ø¡",
    tabTreasury: "Ø§Ù„Ø®Ø²ÙŠÙ†Ø© ÙˆØ§Ù„Ø¨Ù†ÙˆÙƒ", tabExpenses: "Ø§Ù„Ù…ØµØ±ÙˆÙØ§Øª", tabReports: "Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ± Ø§Ù„Ù…Ø§Ù„ÙŠØ©",
    monthRevenue: "Ø£Ø±Ø¨Ø§Ø­ Ø§Ù„Ø´Ù‡Ø± Ø§Ù„Ø­Ø§Ù„Ù‰", monthExpenses: "Ù…ØµØ±ÙˆÙØ§Øª Ø§Ù„Ø´Ù‡Ø± Ø§Ù„Ø­Ø§Ù„Ù‰",
    monthNetProfit: "ØµØ§ÙÙ‰ Ø±Ø¨Ø­ Ø§Ù„Ø´Ù‡Ø±", totalTreasuryBalance: "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø±ØµÙŠØ¯ Ø§Ù„Ø®Ø²ÙŠÙ†Ø© ÙˆØ§Ù„Ø¨Ù†ÙˆÙƒ",
    totalOwedSuppliers: "Ù…Ø³ØªØ­Ù‚ Ù„Ù„Ù…ÙˆØ±Ø¯ÙŠÙ†", totalOwedCustomers: "Ù…Ø³ØªØ­Ù‚ Ù…Ù† Ø§Ù„Ø¹Ù…Ù„Ø§Ø¡",
    profitBySection: "Ø£Ø±Ø¨Ø§Ø­ Ø§Ù„Ø´Ù‡Ø± Ø­Ø³Ø¨ Ø§Ù„Ù‚Ø³Ù…",
    searchSupplier: "Ø§Ø¨Ø­Ø« Ø¹Ù† Ù…ÙˆØ±Ø¯...", searchCustomer: "Ø§Ø¨Ø­Ø« Ø¹Ù† Ø¹Ù…ÙŠÙ„...",
    colSupplier: "Ø§Ù„Ù…ÙˆØ±Ø¯", colCustomer: "Ø§Ù„Ø¹Ù…ÙŠÙ„", colSections: "Ø§Ù„Ø£Ù‚Ø³Ø§Ù…",
    colTotalOwed: "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ­Ù‚", colTotalDue: "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ­Ù‚",
    colPaid: "Ø§Ù„Ù…Ø¯ÙÙˆØ¹", colCollected: "Ø§Ù„Ù…Ø­ØµÙ„", colRemaining: "Ø§Ù„Ù…ØªØ¨Ù‚Ù‰",
    noSuppliers: "Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ù…ÙˆØ±Ø¯ÙˆÙ†", noCustomers: "Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ø¹Ù…Ù„Ø§Ø¡",
    accountsAndTreasuries: "Ø§Ù„Ø­Ø³Ø§Ø¨Ø§Øª ÙˆØ§Ù„Ø®Ø²Ø§Ø¦Ù†", addAccount: "Ø¥Ø¶Ø§ÙØ© Ø­Ø³Ø§Ø¨",
    noAccountsYet: "Ù„Ø§ ØªÙˆØ¬Ø¯ Ø­Ø³Ø§Ø¨Ø§Øª Ø¨Ø¹Ø¯. Ø£Ø¶Ù Ø®Ø²ÙŠÙ†Ø© Ù†Ù‚Ø¯ÙŠØ© Ø£Ùˆ Ø­Ø³Ø§Ø¨ Ø¨Ù†ÙƒÙ‰ Ù„Ù„Ø¨Ø¯Ø¡.",
    treasuryMovement: "Ø­Ø±ÙƒØ© Ø§Ù„Ø®Ø²ÙŠÙ†Ø©", allAccounts: "ÙƒÙ„ Ø§Ù„Ø­Ø³Ø§Ø¨Ø§Øª", manualEntry: "Ù‚ÙŠØ¯ ÙŠØ¯ÙˆÙ‰",
    colDate: "Ø§Ù„ØªØ§Ø±ÙŠØ®", colAccount: "Ø§Ù„Ø­Ø³Ø§Ø¨", colStatement: "Ø§Ù„Ø¨ÙŠØ§Ù†", colAmount: "Ø§Ù„Ù…Ø¨Ù„Øº",
    noTransactions: "Ù„Ø§ ØªÙˆØ¬Ø¯ Ø­Ø±ÙƒØ§Øª", allCategories: "ÙƒÙ„ Ø§Ù„ØªØµÙ†ÙŠÙØ§Øª", addExpense: "Ø¥Ø¶Ø§ÙØ© Ù…ØµØ±ÙˆÙ",
    colCategory: "Ø§Ù„ØªØµÙ†ÙŠÙ", colDescription: "Ø§Ù„ÙˆØµÙ", noExpenses: "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…ØµØ±ÙˆÙØ§Øª",
    rangeToday: "Ø§Ù„ÙŠÙˆÙ…", rangeMonth: "Ø§Ù„Ø´Ù‡Ø± Ø§Ù„Ø­Ø§Ù„Ù‰", rangeCustom: "Ù…Ø®ØµØµ", to: "Ø¥Ù„Ù‰",
    exportExcel: "ØªØµØ¯ÙŠØ± Excel", revenueOf: (s) => `Ø¥ÙŠØ±Ø§Ø¯Ø§Øª ${s}`, bookingsCount: "Ø­Ø¬Ø²",
    totalRevenue: "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª", totalExpenses: "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø§Ù„Ù…ØµØ±ÙˆÙØ§Øª", netProfit: "ØµØ§ÙÙ‰ Ø§Ù„Ø±Ø¨Ø­",
    expensesByCategory: "Ø§Ù„Ù…ØµØ±ÙˆÙØ§Øª Ø­Ø³Ø¨ Ø§Ù„ØªØµÙ†ÙŠÙ", noExpensesInPeriod: "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…ØµØ±ÙˆÙØ§Øª ÙÙ‰ Ù‡Ø°Ù‡ Ø§Ù„ÙØªØ±Ø©",
    editExpense: "ØªØ¹Ø¯ÙŠÙ„ Ù…ØµØ±ÙˆÙ", descriptionOptional: "Ø§Ù„ÙˆØµÙ (Ø§Ø®ØªÙŠØ§Ø±Ù‰)", amountEgp: "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)",
    payFromAccount: "ØµØ±Ù Ù…Ù† (Ø®Ø²ÙŠÙ†Ø©/Ø­Ø³Ø§Ø¨)", selectAccount: "Ø§Ø®ØªØ± Ø­Ø³Ø§Ø¨", notes: "Ù…Ù„Ø§Ø­Ø¸Ø§Øª",
    saveChanges: "Ø­ÙØ¸ Ø§Ù„ØªØ¹Ø¯ÙŠÙ„",
    editAccount: "ØªØ¹Ø¯ÙŠÙ„ Ø­Ø³Ø§Ø¨", addAccountTreasury: "Ø¥Ø¶Ø§ÙØ© Ø­Ø³Ø§Ø¨/Ø®Ø²ÙŠÙ†Ø©", accountName: "Ø§Ø³Ù… Ø§Ù„Ø­Ø³Ø§Ø¨",
    accountNamePlaceholder: "Ù…Ø«Ø§Ù„: Ø®Ø²ÙŠÙ†Ø© Ø§Ù„Ù…ÙƒØªØ¨ØŒ Ø­Ø³Ø§Ø¨ Ø¨Ù†Ùƒ Ù…ØµØ±", type: "Ø§Ù„Ù†ÙˆØ¹",
    openingBalance: "Ø§Ù„Ø±ØµÙŠØ¯ Ø§Ù„Ø§ÙØªØªØ§Ø­Ù‰ (Ø¬.Ù…)",
    manualEntryTitle: "Ù‚ÙŠØ¯ ÙŠØ¯ÙˆÙ‰", directionIn: "ÙˆØ§Ø±Ø¯ (+)", directionOut: "Ù…Ù†ØµØ±Ù (-)",
    item: "Ø§Ù„Ø¨Ù†Ø¯", saveEntry: "Ø­ÙØ¸ Ø§Ù„Ù‚ÙŠØ¯",
    recordNewPayment: "ØªØ³Ø¬ÙŠÙ„ Ø¯ÙØ¹Ø© Ø¬Ø¯ÙŠØ¯Ø©", payFrom: "Ø§Ø¯ÙØ¹ Ù…Ù† (Ø®Ø²ÙŠÙ†Ø©/Ø­Ø³Ø§Ø¨)",
    notesOptional: "Ù…Ù„Ø§Ø­Ø¸Ø§Øª (Ø§Ø®ØªÙŠØ§Ø±Ù‰)", recordPayment: "ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø¯ÙØ¹Ø©",
    paymentHistory: "Ø³Ø¬Ù„ Ø§Ù„Ù…Ø¯ÙÙˆØ¹Ø§Øª", noPaymentsRecorded: "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ø¯ÙÙˆØ¹Ø§Øª Ù…Ø³Ø¬Ù„Ø©",
    relatedBookings: "Ø§Ù„Ø­Ø¬ÙˆØ²Ø§Øª Ø§Ù„Ù…Ø±ØªØ¨Ø·Ø©", noBookings: "Ù„Ø§ ØªÙˆØ¬Ø¯ Ø­Ø¬ÙˆØ²Ø§Øª",
    recordNewCollection: "ØªØ³Ø¬ÙŠÙ„ ØªØ­ØµÙŠÙ„ Ø¬Ø¯ÙŠØ¯", collectInto: "Ø§Ù„ØªØ­ØµÙŠÙ„ ÙÙ‰ (Ø®Ø²ÙŠÙ†Ø©/Ø­Ø³Ø§Ø¨)",
    recordCollection: "ØªØ³Ø¬ÙŠÙ„ Ø§Ù„ØªØ­ØµÙŠÙ„", collectionHistory: "Ø³Ø¬Ù„ Ø§Ù„ØªØ­ØµÙŠÙ„Ø§Øª",
    noCollectionsRecorded: "Ù„Ø§ ØªÙˆØ¬Ø¯ ØªØ­ØµÙŠÙ„Ø§Øª Ù…Ø³Ø¬Ù„Ø©",
    currency: "Ø¬.Ù…",
    confirmDeleteExpense: "Ù‡Ù„ ØªØ±ÙŠØ¯ Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„Ù…ØµØ±ÙˆÙØŸ",
    confirmDeleteAccount: "Ù‡Ù„ ØªØ±ÙŠØ¯ Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„Ø­Ø³Ø§Ø¨ØŸ Ù„Ù† ÙŠØªÙ… Ø­Ø°Ù Ø§Ù„Ø­Ø±ÙƒØ§Øª Ø§Ù„Ù…Ø³Ø¬Ù„Ø© Ø¹Ù„ÙŠÙ‡ Ù…Ù† Ù‚Ø¨Ù„.",
    confirmDeleteEntry: "Ù‡Ù„ ØªØ±ÙŠØ¯ Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„Ù‚ÙŠØ¯ØŸ",
    confirmDeletePayment: "Ù‡Ù„ ØªØ±ÙŠØ¯ Ø­Ø°Ù Ù‡Ø°Ù‡ Ø§Ù„Ø¯ÙØ¹Ø©ØŸ",
    confirmDeleteCollection: "Ù‡Ù„ ØªØ±ÙŠØ¯ Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„ØªØ­ØµÙŠÙ„ØŸ",
  },
  en: {
...
const getEmptyForm = () => ({
  ...
  from: "",
  to: "",
  // Return-leg airport, shown only for a round trip â€” the outbound "to" airport is
  ...
  destinations: ["", ""],
  // "legs": each flight leg has its own independent From/To pair (see legsFromPairs) â€”
  ...
  tripType: "oneWay",
  airline: "",
  // Flight number (e.g. "MS985") â€” optional, used only to look up the flight via
  ...
  // as Hotels/Visa/Transfers) â€” converted to EGP for totals using the shared
  // USD -> EGP rate in the header.
  netCurrency: "EGP",
  soldCurrency: "EGP",
  // How the Net price is being paid/settled â€” Cash, Client CC, or Office CC.
  netPaymentMethod: "cash",
  // Per-passenger rates for Child/Infant customers (see ticketPaxCounts/ticketNetTotal
  // above) â€” same currency as netCurrency/soldCurrency, only used/shown once at least
  ...
  oldTicketNumber: "",
  oldTicketIssueDate: "",
  // EMD (Electronic Miscellaneous Document) tied to the exchange â€” shown only while
  ...
  emdNumber: "",
  emdAmount: "",
  // Refund tracking: a list of refund records (each with two amounts â€” refunded by the
  ...
  // customers/tickets, so this is a list â€” one entry per refunded customerIndex â€” rather
...
// owns its own two cells â€” unlike an older "chain of points" layout, editing one leg's
...
// Max number of adult guests a room type can hold â€” drives how many guest-name fields
...
// A child staying in a room, with an age (in whole years, 0â€“11) alongside the name.
const emptyChild = () => ({
...
// Converts Arabic-Indic (Ù -Ù©) and Extended Arabic-Indic (Û°-Û¹) digits to standard 0-9,
...
// How the Net price (what's owed/paid to the supplier/airline) was or will be settled â€”
...
// currency, count, and net/sold price per room per night â€” e.g. "1x Single, Half Board,
...
const emptyRoomLine = () => ({
  ...
  // Adult guest names â€” sized to the default room type's capacity (single -> 1).
  guests: guestsForCapacity([], ROOM_CAPACITY.single),
  // Children staying in this room, each with a name and age (0â€“11 years).
  children: [],
});
...
const getEmptyHotelForm = () => ({
  ...
  // The date the reservation itself was made â€” separate from each room's own
  // check-in/check-out dates below.
  bookingDate: todayDateStr(),
  notes: "",
});

// A single customer on a visa booking â€” just a name (visas don't track per-customer
...
// names already entered â€” same rationale as resizeCustomers() above.
...
// 999 back to 000); everything before them â€” including the rest of the serial â€” stays fixed,
...
// conjunction ticket â€” the customer's second ticket number issued together with the
...
// its tail rather than after the first ticket's tail â€” e.g. first ticket
...
// ROLE_PRESETS), but every toggle can still be switched on or off by hand afterwards â€”
...
// Accountant/Owner â€” the all-section Manager grade has been removed), there's a grade
// per section per tier â€” e.g. "Flights Employee", "Tourism Supervisor", "Transportation
// Manager" â€” that automatically limits the employee to that one section (every other
...
// Same idea for the per-section Supervisor grade â€” "Ticketing Supervisor" instead of
...
// Same idea for the per-section Manager grade â€” "Ticketing Manager" instead of
...
// grade â€” Files is the one exception for all three, since it's normally reached
...
// isOwner, checked alongside currentUser.isAdmin wherever those are gated) â€” the one
// thing an Owner or GM never gets is the License panel, which stays reserved for true
// main accounts. GM is defined as an exact copy of the Owner preset â€” same starting
// permissions, same isOwner flag â€” it's simply a second, identically-privileged grade.
...
const ROLE_PRESETS = {
  ...
  // rights on top of it â€” the senior grade within the accounting tier.
...
// department's three tiers (Manager, Supervisor, Employee) â€” e.g. the Flights
...
// see and use, independent of their ticket permissions (view/add/edit/delete) above â€” an
...
// Fine-grained permissions â€” View all services / Edit / Delete â€” set independently for
...
// services off, which simply limits them to editing their own records. Every section â€”
// Flights, Hotels, Visa, Transportation, and Files â€” tracks which employee created each
...
// canViewAll/canEdit/canDelete values â€” permissions stay exactly as they were until the
...
const emptyNewEmployee = {
  ...
  // full access to every closed year regardless of this map â€” it only matters for
...
// fully visible and easy to use â€” this is the one place permissions for an existing
...
const EmployeePermissionsModal = ({ emp, onClose, onSetRole, onSetPermission, onSetSection, onSetSectionPerm, onSave, onDelete }) => {
  // Name/username/password are edited right here, not in the employee table â€” the
...
// IATA 3-digit airline accounting/ticketing prefix codes â€” the first 3 digits of a
...
const AIRLINE_CODES = [
  ...
  // Additional airlines â€” mainly Egyptian carriers and other regional/international
  ...
  // `code` is left null for these â€” they still work for selection/search, but
...
// 2-letter IATA airline designator (e.g. "MS" for EgyptAir) â€” this is what gets
...
// distinct values â€” an Excel-style "filter inside the table" control, as opposed
...
// each removable, plus a trailing "Clear all"). Every list in the app â€” tickets,
// hotels, visas, transfers, files â€” used to hand-roll this same block; now they all
...
// pair â€” the common case (year, month, company, employee, supplier, airline, ...).
...
function TicketsApp({ onChangeServer, currentServerUrl } = {}) {
  ...
  useEffect(() => {
    document.title = "Travel Agency Management";
    const faviconSvg =
  ...
  // employees never see this â€” it's gated to currentUser.isAdmin wherever it's shown.
  ...
  // the JSX further down â€” which still reads the destructured local variables
  // licenseRecord, showLicensePanel, licenseInput, licenseError, licenseSaving,
  // isLicensed â€” needed NO changes. Only the *setters* changed, to dispatch(...)
  ...
  const handleActivateLicense = async () => {
    ...
    try {
      ...
      dispatchLicense({ error: "Couldn't save the activation code â€” please try again.", saving: false });
    }
  ...
  // In-memory only â€” NEVER persisted anywhere. Unwrapped from the logged-in employee's
  // keyWrap at login time; lets secureLoad/secureSave decrypt and encrypt customer and
  // financial records. Lost on every page reload by design (see handleLogin) â€” that's
  ...
  // feature, or was created before a workspace key existed) â€” an admin needs to reset
  ...
  // Pass an `onUndo` to add an Undo button to the toast (used after deletes) â€” the toast
  ...
  // relevant while actionToast.onUndo is set â€” null the rest of the time.
  ...
  // data in sync â€” that's also how a teammate's screen finds out about a new request
  // addressed to them without a manual refresh.
  const [requests, setRequests] = useState([]);
  const [showRequestsPanel, setShowRequestsPanel] = useState(false);

  // ---------- Closed years (per section) ----------
  // A year "closed" for a given section is fully hidden â€” from lists, filter options,
  // stats, and exports alike â€” for anyone who isn't Admin/Owner/GM/Accounts. Stored
  // centrally (shared, unencrypted â€” just a list of year strings, not sensitive data)
  ...
  // years panel â€” reset each time the panel is reopened, see the reset effect below.
  ...
  const [showPassword, setShowPassword] = useState(false);
  // While an account with the authenticator app enabled is signing in: holds the
  // matched employee record + the password they just typed (already verified), so the
  // login screen can switch to an OTP step before actually finishing the sign-in.
  ...
  // prompt. Re-entering the SAME employee's password is required to dismiss it â€”
  ...
  // own trigger+panel container â€” not the whole Grade block. Clicking a WIDER area
  ...
  // Global "Manage suppliers" panel â€” lets an admin/owner add or remove supplier names
  ...
  // "Management" flyout menu in the header â€” groups the account/admin icon buttons
  ...
  // Two-factor (authenticator app) setup/disable, scoped to the signed-in employee's
  // own account â€” managed alongside the password-change modal above.
  ...
  // (non-native) dropdown â€” rather than a plain <select> â€” so each option can show that
  ...
  // Flight lookup â€” looks up a flight number via the AviationStack API to
  ...
  // already has it â€” nobody has to paste their own key.
  ...
  // AviationStack flight lookup above does. No API key needed â€” this calls the
  ...
  // Whether the Visa page's own "Add supplier" panel is open, plus its draft text â€”
  ...
  // Visa requirement checker â€” looks up entry rules for a passport/destination
  ...
  // Whether the "other" free-text supplier field is shown instead of the dropdown list â€”
  ...
  // together. Each item only stores which record it points to â€” its label/date/price is
  ...
  // seconds later, by which point other edits may have landed â€” so it can't safely close
  ...
  // otherwise { company, notes, createdAt, items }. Lives only in local state â€” nothing is
  ...
  // Set when "copy to a file" is clicked from the Flights/Hotels/Visa tables directly â€”
  // { type: 'flights'|'hotels'|'visa', record } â€” opens a modal asking which file (by
  ...
  // and a separate "issued ticket value" box â€” entering an amount there deducts it from
  ...
  // iataBalanceLoaded stays false until the saved balance has actually been fetched â€”
  ...
  // History of today's deductions from the IATA balance only â€” starts empty again at
  ...
  // values (expense categories, entry categories, etc.) always stay in Arabic â€” this
  ...
  // Supplier/customer ledgers are derived, not stored â€” drilling into one just
  ...
  // row â€” including an old, reissued ticket that's normally hidden from the table â€” can
  ...
  // once â€” each row is its own ticket-number lookup plus its own amounts.
  ...
  // Search + filter state for the Hotels, Visa, Transportation, and Files sections â€”
  ...
  // Once true, the first-run setup screen must never be shown again â€” even if the employee
  // list later becomes empty (e.g. accounts deleted, a bad restore) â€” so no one can
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // payments/treasury) can't actually be decrypted yet at this point â€” nobody
        ...
        if (suggestionsRes && suggestionsRes.value) {
          try {
            ...
            setSuggestions({
              ...
              customers: [], // never restore saved customer names â€” this field must have no autocomplete history
        ...
        // sessionStorage first â€” see saveLocalSession/loadLocalSession above. This is what
        ...
        if (localMatch) {
          ...
          if (localSession) clearLocalSession(); // stale â€” account no longer exists
          if (sessionRes && sessionRes.value) {
            ...
            if (match) {
              ...
              // tab, or sessionStorage was cleared): just pre-fill the username â€” the
  ...
  useEffect(() => {
    ...
    const loadCoreData = async () => {
      ...
      try {
        ...
        // These 10 collections are encrypted at rest â€” secureLoad returns { value: null }
        ...
        // this browser actually saw â€” that's what lets it detect another employee's save
        ...
        // flightApiKeyData/visaApiKeyData can legitimately be "" (key removed) â€”
  ...
  // critical path entirely â€” a failure or slow response here can never block or break
  ...
  useEffect(() => {
    ...
    const loadClosedYears = async () => {
      try {
        ...
        // Key doesn't exist yet (nobody has closed a year) or a transient read error â€”
  ...
  const myActivity = (() => {
    ...
    if (activeSection === "hotels") {
      if (viewingHotelBooking) {
        return `Viewing hotel booking â€” ${viewingHotelBooking.hotel || "hotel"}${
          viewingHotelBooking.customer ? ` (${viewingHotelBooking.customer})` : ""
        }`;
      }
      if (hotelEditingId) {
        const hb = hotelBookings.find((h) => h.id === hotelEditingId);
        return `Editing hotel booking â€” ${(hb && hb.hotel) || "hotel"}`;
      }
    ...
    if (activeSection === "visa") {
      if (visaEditingId) {
        ...
        return `Editing visa booking â€” ${(vb && vb.visaType) || "visa"}`;
      }
    ...
    if (viewingTicketId) {
      ...
      return `Viewing ticket${name ? ` â€” ${name}` : ""}${vt && vt.company ? ` (${vt.company})` : ""}`;
    }
    if (form.id) {
      const name = form.customers && form.customers[0] && form.customers[0].name;
      return `Editing ticket${name ? ` â€” ${name}` : ""}${form.company ? ` (${form.company})` : ""}`;
    }
    ...
    if (draftName || form.company) {
      return `Adding a new ticket${draftName ? ` â€” ${draftName}` : ""}${form.company ? ` (${form.company})` : ""}`;
    }
    return "Browsing flights list";
  })();
  // Kept in a ref (rather than read directly) so the heartbeat interval below â€” which
  // only re-subscribes when currentUser changes â€” always sends the latest activity
  ...
  // below). Every signed-in client â€” including this one â€” checks its own flag on each
  ...
  // touch the saved session here â€” "beforeunload"/"pagehide" also fire on a normal page
  ...
  // array under one storage key â€” so without this check, whoever saves second would
  ...
  // save). Before writing, we peek the current stored version (cheap â€” no decrypt, see
  ...
  const CONFLICT_MESSAGE = "This data was just updated by someone else â€” refresh the page and try again";
  ...
  const fetchUsdRateOnline = async () => {
    ...
    try {
      ...
      setFetchUsdRateError("Couldn't fetch the rate â€” enter it manually");
    } finally {
      setFetchingUsdRate(false);
    }
  };

  // IATA balance â€” same shared-storage pattern as the USD rate above, so every signed-in
  // employee sees the same running balance. iataBalanceLoaded is set true once this fetch
  // settles (found a value or not) â€” deductions are blocked until then, see
  ...
  // only ever holds today's deductions â€” if the saved entry is from an earlier day, it's
  ...
  // Logs one deducted amount under today's date. The log holds only today's deductions â€”
  ...
  // deductions are logged here â€” manually overwriting the balance box itself is not.
  ...
  // Pressing Enter in that box (see onKeyDown below) is the only way this fires â€”
  ...
  // balance â€” the balance itself only ever changes here or when typed directly by hand.
  ...
  // removed too â€” otherwise the file keeps showing a stale line that errors out when
  ...
  // original service â€” that behavior is unchanged.
  ...
  // (sourceType, sourceId) â€” captured right before removeItemFromAllFiles strips them,
  ...
  // shared login-history log. Best-effort and silent on failure â€” a logging hiccup should
  ...
  const recordLogin = async (user, type = "login") => {
    ...
    try {
      ...
      // Login history is a convenience/audit feature â€” failures here must never
  ...
  // failure â€” a logging hiccup should never block the actual action from completing.
  ...
  const recordActivity = async (module, action, description) => {
    ...
    try {
      ...
      // Activity log is an audit convenience feature â€” failures here must never
  ...
  // Saves the full closed-years map and updates local state immediately (optimistic â€”
  ...
  const persistClosedYears = async (next) => {
    ...
    try {
      ...
      // Silent â€” worst case the toggle re-syncs on the next poll.
    }
  };
  ...
  // `section` (flights/hotels/visa/cars) â€” once a year is closed, records dated in it
  ...
  // intentionally excluded â€” a new company can only be registered via the
  ...
  const rememberSuggestionsFromRecord = (record) => {
    ...
    let next = {
      ...
      // Customer names are intentionally never remembered here â€” the customer field
  ...
  // company's full details â€” name, tax number, commercial registration number, phone
  // numbers, and any special deal/agreement terms â€” so they're always available to pick
  ...
  // (not saved yet â€” only committed when the company itself is saved). A company can
  ...
  // recorded under that company name are untouched â€” this only affects the picker.
  ...
  const handleSaveHotel = async () => {
    setHotelError("");
    // Company name and supplier are both optional â€” a blank company means an Individual
    ...
    for (const l of lines) {
      ...
      // Only the first guest in each room is required â€” the rest are optional.
    ...
    // A closed year blocks every add/edit â€” whether the booking already belongs to that
    ...
    if ((originalHotel && isYearLocked("hotels", originalHotel.bookingDate) && !canEditClosedYear((originalHotel.bookingDate || "").slice(0, 4))) || (isYearLocked("hotels", hotelForm.bookingDate) && !canEditClosedYear((hotelForm.bookingDate || "").slice(0, 4)))) {
      setHotelError("This year is closed for accounting â€” bookings dated in a closed year can't be added or edited. Ask a General Manager or Admin to reopen the year first.");
      return;
    }
    ...
    if (hotelEditingId) {
      ...
      const record = {
        ...
        // Locked in once, the first time this booking is saved â€” see the same note
  ...
  const handleEditHotelClick = (h) => {
    ...
    setHotelForm({
      ...
      // both net and sold â€” fall back to it so older records still load sensibly.
      ...
      roomLines:
        Array.isArray(h.roomLines) && h.roomLines.length > 0
          ? h.roomLines.map((l) => ({
              ...
              // Legacy bookings kept dates on the booking itself rather than per room â€”
              ...
              // Legacy bookings had no guest names â€” pad an empty list to match capacity.
  ...
  // brand-new booking â€” the original booking is left untouched. Unlike
  ...
  // containers for those names, while everything else â€” hotel, supplier, dates,
  // currencies, prices, notes â€” starts fresh, same as a brand-new booking.
  ...
  // Registers a new supplier name in the Flights page's OWN supplier list â€” kept
  ...
  const handleSaveVisa = async () => {
    ...
    if ((originalVisa && isYearLocked("visa", originalVisa.bookingDate) && !canEditClosedYear((originalVisa.bookingDate || "").slice(0, 4))) || (isYearLocked("visa", visaForm.bookingDate) && !canEditClosedYear((visaForm.bookingDate || "").slice(0, 4)))) {
      setVisaError("This year is closed for accounting â€” bookings dated in a closed year can't be added or edited. Ask a General Manager or Admin to reopen the year first.");
      return;
    }
    if (visaEditingId) {
  ...
  const handleEditVisaClick = (v) => {
    ...
    setVisaForm({
      ...
      visaType: v.visaType || "",
      supplier: v.supplier || "",
      // Legacy bookings stored one currency for both net and sold â€” fall back to it.
  ...
  // Registers a new supplier name in the Visa page's OWN supplier list â€” kept separate
  ...
  const handleSaveCar = async () => {
    ...
    if ((originalCar && isYearLocked("cars", originalCar.bookingDate) && !canEditClosedYear((originalCar.bookingDate || "").slice(0, 4))) || (isYearLocked("cars", carForm.bookingDate) && !canEditClosedYear((carForm.bookingDate || "").slice(0, 4)))) {
      setCarError("This year is closed for accounting â€” bookings dated in a closed year can't be added or edited. Ask a General Manager or Admin to reopen the year first.");
      return;
    }
    if (carEditingId) {
      const commitCar = async () => {
        ...
        recordActivity("Transportation", "edited", `Edited car booking: ${carForm.customerName || "customer"} (${carForm.routeFrom || "?"} â†’ ${carForm.routeTo || "?"})`);
        resetCarForm();
      };
      ...
      recordActivity("Transportation", "created", `Created car booking: ${record.customerName || "customer"} (${record.routeFrom || "?"} â†’ ${record.routeTo || "?"})`);
    }
    resetCarForm();
  };
  ...
  const handleDeleteCar = (id, onDeleted) => {
    ...
    requestConfirm("Delete this transfer booking?", async () => {
      ...
      if (deleted) {
        recordActivity("Transportation", "deleted", `Deleted car booking: ${deleted.customerName || "customer"} (${deleted.routeFrom || "?"} â†’ ${deleted.routeTo || "?"})`);
        showActionToast("Transfer booking deleted", async () => {
          await persistCarBookings([deleted, ...carBookingsRef.current]);
          await restoreFileItemLinks(linkedFileEntries);
          recordActivity("Transportation", "restored", `Restored car booking: ${deleted.customerName || "customer"} (${deleted.routeFrom || "?"} â†’ ${deleted.routeTo || "?"})`);
        });
      }
  ...
  // an array of { heading, rows: [[label, value], ...] } â€” keeping this in one place
  ...
  const buildReceiptHtml = (docTitle, subtitle, sections) => {
    ...
    return `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <img src="${LOGO_DATA_URL}" alt="Travel Agency Management" />
            <div>
              <h1>${subtitle}</h1>
              <p>Travel Agency Management</p>
            </div>
          </div>
          ${sectionsHtml}
          <div class="footer">
  ...
  // browser tab/window â€” this is the in-app "popup system" used for every printable
      ...
      },
      {
        ...
        rows: [
          ...
          ...((t.netCurrency === "USD" || t.soldCurrency === "USD") && t.usdRate
            ? [["USD â†’ EGP rate used", `${fmt(t.usdRate)} (locked at booking)`]]
            : []),
        ],
      },
      ...(hasRefund(t)
      ...
      ...roomSections,
      {
        ...
        rows: [
          ...
          ...((h.netCurrency === "USD" || h.soldCurrency === "USD") && h.usdRate
            ? [["USD â†’ EGP rate used", `${fmt(h.usdRate)} (locked at booking)`]]
            : []),
        ],
      },
    ]);
        ...
        },
        {
          ...
          rows: [
            ...
            ...((v.netCurrency === "USD" || v.soldCurrency === "USD") && v.usdRate
              ? [["USD â†’ EGP rate used", `${fmt(v.usdRate)} (locked at booking)`]]
              : []),
          ],
        },
      ]
  ...
  // Opens a printable summary of a whole File â€” its info plus every item it
  ...
  const handlePrintFile = (f) => {
    ...
    const itemRows = (f.items || []).map((it) => {
      ...
      return [
        `${FILE_SOURCE_LABELS[it.sourceType] || it.sourceType} â€” ${r.label}`,
  ...
  // Registers a new supplier name in the Transfers page's OWN supplier list â€” kept
  ...
  const handleCreateFirstAdmin = async () => {
    ...
    // admin â€” see the "Workspace encryption" section near the top of this file.
  ...
  // Everything that finishes a login once the password (and, if enabled, the
  // authenticator code) has already been verified. Split out of handleLogin so the OTP
  // step can call the exact same completion logic after a successful code check.
  const completeLogin = async (match, plainPassword) => {
    setLoginFailCount(0);
    setLoginLockUntil(0);
    setKeyAccessWarning("");
    ...
    // we have the plaintext in hand. Invisible to the employee â€” just a stronger
    ...
    if (match.keyWrap) {
      const unwrapped = await unwrapWorkspaceKey(match.keyWrap, plainPassword);
      if (unwrapped) {
      ...
      const workspaceKeyObj = await generateWorkspaceKey();
      const wrap = await wrapWorkspaceKey(workspaceKeyObj, plainPassword);
      await persistEmployees((employees || []).map((e) => (e.username === match.username ? { ...e, keyWrap: wrap } : e)));
      resolvedWorkspaceKey = workspaceKeyObj;
      setWorkspaceKey(workspaceKeyObj);
    } else {
      setKeyAccessWarning("Your account doesn't have access to encrypted data yet â€” ask an admin to reset your password once (Manage Employees) to enable it.");
    }
  ...
  const handleLogin = async () => {
    ...
    // Password is correct. If this account has the authenticator app enabled, hold off
    // on finishing the sign-in â€” keep the verified password around just long enough to
    // finish completeLogin once the code checks out â€” and switch the form to the OTP step.
    if (match.totpEnabled && match.totpSecret) {
      setPendingTotpLogin({ match, loginPassword });
      setOtpCodeInput("");
      setOtpError("");
      return;
    }
    await completeLogin(match, loginPassword);
  };

  const handleVerifyLoginOtp = async () => {
    if (!pendingTotpLogin) return;
    setOtpError("");
    const ok = await verifyTotpCode(pendingTotpLogin.match.totpSecret, otpCodeInput);
    if (!ok) {
      setOtpError("Incorrect code. Check your authenticator app and try again.");
      return;
    }
    await completeLogin(pendingTotpLogin.match, pendingTotpLogin.loginPassword);
  };

  const cancelTotpLogin = () => {
    setPendingTotpLogin(null);
    setOtpCodeInput("");
    setOtpError("");
    setLoginPassword("");
  ...
  // Covers the screen with a password prompt without ending the session â€” the
  ...
  // Visa requirement checker â€” saves the RapidAPI key into the same encrypted
  ...
  const handleSaveVisaApiKey = async () => {
    ...
    try {
      ...
      setVisaCheckError("Couldn't save the key to shared storage â€” try again.");
    }
  };
  ...
  const handleClearVisaApiKey = async () => {
    ...
    try {
      ...
      // Best-effort â€” the key still gets cleared locally either way.
    }
  };
  ...
  // Flight lookup â€” saves the AviationStack key into the same encrypted shared
  ...
  const handleSaveFlightApiKey = async () => {
    ...
    try {
      ...
      setFlightLookupError("Couldn't save the key to shared storage â€” try again.");
    }
  };
  ...
  // Triggered from the ticket form's own flight-number field â€” looks the flight up
  ...
  // fully client-side replacement for the old Claude-Vision-based scan â€” no
  ...
  // as text on the ticket (e.g. "CAI") â€” unlike the AI version, it cannot
  ...
  const extractTicketFieldsFromText = (raw) => {
    ...
    const result = {
      ...
      // routing (see the stops-walking logic below) â€” a plain A-B-A round
    ...
    // which GDS the ticket came from â€” sets the Supplier field automatically.
    ...
    // Ticket number: "TKT-3814830484393" â€” label makes this more reliable
    ...
    // internal dash. That shape is unique to this header field â€” every other
    ...
    // Passenger name: numbered row "1.LASTNAME/FIRST MIDDLE MADT" â€” the
    ...
    // us the passenger type â€” an infant ticket prints "INF" here with no
    // name suffix otherwise distinguishing it â€” so it also drives which
    ...
    // until it reaches the actual type code â€” the title is then stripped
    ...
    if (gdsNameMatch) {
      ...
      // some layouts may omit it) â€” still try to recover at least the name,
    ...
    //   "1 OCAI XY 584 ..."   â€” a flown leg; "O"+3 letters is its origin
    //   "2 JED ARNK"          â€” a surface/ARNK gap (no flight); the 3 letters
    //                           are where that gap starts (passenger makes
    //                           their own way to the next segment's origin)
    //   "CAI"                 â€” a lone 3-letter line: the arrival city of the
    //                           segment just above, printed on its own line
    //                           only when it wrapped (this is how the FINAL
    //                           arrival city â€” never itself a segment origin
    //                           â€” shows up at all)
    ...
    // arrival) â€” that distinction is what lets the leg-building step below
    ...
    // "multi-city" the routing is â€” that's what makes a plain CAI-JED-CAI
    ...
    // segment's origin â€” its destination is simply the next waypoint in the
    ...
    // just two legs, CAI-JED and MED-CAI â€” not a fabricated third leg
    ...
    // right source, not the flight's own departure date â€” those are two
    ...
    // above (distinctCore.length > 2) â€” that check already walked the same
    ...
    // ticket's data â€” e.g. "FO 077-6907998436ALY30APR26/90218376/077-6907998566"
    ...
    // Total price: "TOTAL   EGP      12322.80" â€” this is the full price the
    ...
    // loose separator between the currency code and the amount â€” OCR often drops
    ...
    // digits) regardless of which pattern above found it â€” the TKT- match and
  ...
  // A separate, much smaller heuristic than extractTicketFieldsFromText above â€”
  ...
  const extractEmdFieldsFromText = (raw) => {
    ...
    // by OCR) â€” 3-digit airline prefix + 10-digit number, same shape as a ticket
    ...
    // "TOTAL   EGP      2000.00" â€” on the EMD slip itself (unlike on a reissued
  ...
  // Takes an uploaded screenshot/photo â€” either a ticket mask or an EMD slip â€” runs
  ...
  // on, since an EMD only ever exists for a reissue) â€” the ticket's own fields
    ...
    try {
      ...
      if (looksLikeEmd) {
        ...
        if (!extracted.emdNumber && !extracted.totalAmount) {
          setTicketScanError("Couldn't recognize EMD details in that image â€” please check the fields and fill in anything missing.");
        }
        return;
      }
      ...
      setForm((prev) => {
        ...
        // passenger type â€” an infant or child ticket has its own fare tier
        ...
        // route â€” a plain A-B(-A) ticket leaves this false and only from/to
        ...
        const next = {
          ...
          // Only ever turns reissue ON from a scan, never off â€” if the person already
          ...
          // date are deliberately NOT auto-filled here â€” those stay manual entry only.
        ...
        // separately (upload customer 1's screenshot, then customer 2's, etc.) â€” so
      ...
      if (!extracted.passengerName && !extracted.ticketNumber && !extracted.fromIata && !extracted.toIata) {
        setTicketScanError("Couldn't recognize ticket details in that image â€” please check the fields and fill in anything missing.");
      } else if (!extracted.totalAmount) {
        // Other fields came through fine but the price line itself didn't match â€”
        // flag it specifically so a blank net price doesn't go unnoticed.
        setTicketScanError("Couldn't read the TOTAL amount from that image â€” please enter the net price manually.");
      }
    } catch (err) {
  ...
  // an image â€” pasting text into a normal field is untouched. A ref holds the
  ...
  // lock screen â€” checked the same way as a normal login, against that employee's
    ...
    try {
      const next = [
        ...(employees || []),
        {
          ...
          // workspace key in memory (the admin creating them is logged in) â€” so wrap it
      ...
      // no longer existed) â€” the employee would sometimes still get added underneath,
  ...
  // afterwards via handleTogglePermission â€” the preset is just a fast starting point.
  ...
  // permissions â€” and independent of the other two toggles within the same section too.
  ...
  // year â€” independent of their grade/role, and independent of every other year's
  ...
  const saveEditEmployee = async () => {
    ...
    // a true main account's own credentials â€” that stays admin-to-admin only.
    ...
    // wrapped copy of the workspace key â€” we have their new plaintext right here.
    ...
    // An admin resetting someone ELSE's password is also the recovery path for a
    // lost/broken authenticator device â€” the admin never has the old TOTP secret, so a
    // reset here clears 2FA too; the employee can re-enable it themselves once back in.
    // Resetting your own password (editingUsername === currentUser.username) never
    // touches 2FA â€” that's handled by the dedicated Enable/Disable 2FA controls instead.
    const clearsTotp = !!editDraft.password && editingUsername !== currentUser.username;
    const next = (employees || []).map((e) =>
      e.username === editingUsername
        ? {
            ...e, name: trimmedName, username: trimmedUsername, password: targetPassword, keyWrap: targetKeyWrap,
            ...(clearsTotp ? { totpEnabled: false, totpSecret: null } : {}),
          }
        : e
    );
    await persistEmployees(next);
    recordActivity(
      "Employees",
      "edited",
      `Edited account details for @${trimmedUsername}${clearsTotp ? " (reset password, which also cleared 2FA)" : ""}`
    );
  ...
  // Returns an error message on failure, or null on success â€” the modal shows the
  ...
  const handleSaveEmployeeDetails = async (username, draft) => {
    ...
    // See the matching comment in saveEditEmployee: resetting someone else's password
    // is the recovery path for a lost authenticator device, so it also clears 2FA.
    const clearsTotp = !!draft.password && username !== currentUser.username;
    const next = (employees || []).map((e) =>
      e.username === username
        ? {
            ...e, name: trimmedName, username: trimmedUsername, password: targetPassword, keyWrap: targetKeyWrap,
  ...
  const startTotpSetup = async () => {
    ...
    try {
      ...
      // QR generation is a convenience â€” the manual key below still works either way
    }
  };
  ...
  // Only turns 2FA on after the employee proves they actually scanned/typed the secret
  // into their app correctly, by entering one live code back â€” otherwise a typo while
  // copying the secret could lock them out of their own account on the next sign-in.
  const confirmTotpSetup = async () => {
    setTotpSetupError("");
    const ok = await verifyTotpCode(totpSetupSecret, totpSetupCode);
    if (!ok) {
      setTotpSetupError("That code didn't match. Double-check your phone's clock and try again.");
      return;
    }
    const next = (employees || []).map((e) =>
      e.username === currentUser.username ? { ...e, totpEnabled: true, totpSecret: totpSetupSecret } : e
    );
    await persistEmployees(next);
  ...
  const disableTotp = async () => {
    ...
    setTotpSetupSuccess("");
    const me = (employees || []).find((e) => e.username === currentUser.username);
    if (!me || !(await verifyPassword(me.password, totpDisablePassword))) {
      setTotpDisableError("Password is incorrect");
      return;
    }
    const next = (employees || []).map((e) =>
      e.username === currentUser.username ? { ...e, totpEnabled: false, totpSecret: null } : e
    );
    await persistEmployees(next);
  ...
  // reopening it later never shows a stale in-progress secret, code, or message.
  ...
  const handleRestoreFile = async (e) => {
    ...
    try {
      ...
      const normalizedSuggestions = {
        ...
        // Never restore saved customer names â€” this field must have no autocomplete history.
        customers: [],
      ...
      requestConfirm(
        ...
        async () => {
          ...
          recordActivity(
            "Backup",
            "restored",
            `Restored a backup (${parsed.tickets.length} tickets, ${parsed.employees.length} employees, ${hotelBookingsNext.length} hotel bookings, ${visaBookingsNext.length} visa bookings, ${carBookingsNext.length} car bookings, ${filesNext.length} files) â€” replaced all current data`
          );
          setRestoreSuccess(
  ...
  // the edited one (e.g. "From: CAI â†’ JED"), used to log every ticket edit â€” not just notes â€”
  ...
  const describeTicketChanges = (before, after) => {
    ...
    Object.keys(fieldLabels).forEach((key) => {
      ...
      if (String(beforeVal) !== String(afterVal)) {
        changes.push(`${fieldLabels[key]}: ${beforeVal || "â€”"} â†’ ${afterVal || "â€”"}`);
      }
    });
    ...
    if (beforeCustomers.length !== afterCustomers.length) {
      changes.push(`Customers: ${beforeCustomers.length} â†’ ${afterCustomers.length}`);
    }
    ...
    for (let i = 0; i < maxLen; i++) {
      ...
      if ((b.name || "") !== (a.name || "")) {
        changes.push(`Customer ${i + 1} name: ${b.name || "â€”"} â†’ ${a.name || "â€”"}`);
      }
      if ((b.ticketNumber || "") !== (a.ticketNumber || "")) {
        changes.push(`Customer ${i + 1} ticket number: ${b.ticketNumber || "â€”"} â†’ ${a.ticketNumber || "â€”"}`);
      }
      if ((b.type || "adult") !== (a.type || "adult")) {
        changes.push(`Customer ${i + 1} type: ${PAX_TYPE_LABELS[b.type || "adult"]} â†’ ${PAX_TYPE_LABELS[a.type || "adult"]}`);
      }
    }
  ...
  const handleSubmit = () => {
    ...
    // this check â€” the ticket number stops being mandatory once a PNR is entered.
    ...
    // Each flight leg needs both its own From and To filled in â€” a blank cell can't be
    ...
    // A closed year blocks every add/edit â€” whether the ticket already belongs to that
    // year, or is being dated into it just now.
    if ((original && isYearLocked("flights", original.date) && !canEditClosedYear((original.date || "").slice(0, 4))) || (isYearLocked("flights", form.date) && !canEditClosedYear((form.date || "").slice(0, 4)))) {
      setError("This year is closed for accounting â€” tickets dated in a closed year can't be added or edited. Ask a General Manager or Admin to reopen the year first.");
      return;
    }
    let record = {
      ...form,
      ...
      // Return airport always mirrors the first (From) airport on a round trip â€” it's
      ...
      // shared rate changed â€” an existing lock is always kept, never overwritten.
      usdRate: form.usdRate ?? usdToEgpRate ?? null,
    };
    // Every edit to an existing ticket â€” any field, not just notes â€” gets logged into the
    ...
    const commitTicket = () => {
      ...
      const ticketDesc = `${(record.customers || []).map((c) => c.name).filter(Boolean).join(", ") || "ticket"} (${record.from || "?"} â†’ ${record.to || "?"})`;
  ...
  const handleEdit = (t, afterConfirm) => {
    ...
    // and had no passenger "type" at all â€” every customer without one defaults to Adult.
    const customers =
    ...
    // continuous chain of points rather than per-leg pairs â€” convert those into pairs so
  ...
  const handleDelete = (id, afterConfirm) => {
    ...
    requestConfirm("Delete this ticket?", () => {
      ...
      if (deleted) {
        const ticketDesc = `${(deleted.customers || []).map((c) => c.name).filter(Boolean).join(", ") || "ticket"} (${deleted.from || "?"} â†’ ${deleted.to || "?"})`;
  ...
  const handleToggleVoidTicket = (id) => {
    ...
    const ticketDesc = `${(target.customers || []).map((c) => c.name).filter(Boolean).join(", ") || "ticket"} (${target.from || "?"} â†’ ${target.to || "?"})`;
    requestConfirm(
      nextVoided
  ...
  // an array â€” one entry per refunded customerIndex, since a single booking can have
  ...
  // The recorded refund entry (if any) for one specific customer/ticket within a booking â€”
  ...
  // sides â€” what the airline paid back reduces our cost (net price), and what we paid
  // back to the customer reduces our revenue (sold price) â€” so sales/profit totals
  ...
  // Transfers), so both sides â€” and the refund amounts recorded against them â€” are
  ...
  const handleCustomersCountChange = (value) => {
    ...
    // The PNR reference isn't sequenced like the ticket number â€” every passenger on
    // the same booking shares the same PNR â€” so new rows just inherit the first
  ...
  // Same "CODE - City, Country" â†’ CODE cleanup as handleCityChange, but for one cell
  // (a single leg's From or To) in a multi-destination (multi-city) route. cellIndex is
  // the flat index into form.destinations (leg i's From/To are cells 2*i and 2*i + 1) â€”
  ...
  // Adds a new, independent flight leg with blank From/To â€” it does NOT inherit the
  ...
  const handleCustomerFieldChange = (index, field, value) => {
    ...
    if (field === "ticketNumber") {
      ...
      // (see conjunctionTicketSuffix) â€” not a full independent ticket number, so this
  ...
  // handleCustomerFieldChange this never uppercases the value â€” it's a fixed lowercase
  ...
  const handleCustomerConjunctionToggle = (index, checked) => {
    ...
    // should continue from, so re-run the same cascade as handleTicketNumberBlur below â€”
  ...
  // still empty â€” each one increasing the previous by one. Stops at the first one someone
  ...
  const handleTicketNumberBlur = (index) => {
    ...
    // to them â€” continue the sequence for later customers after ITS tail, not the first
    // ticket's tail.
  ...
  // drives the rest â€” all passengers on the same booking normally share one PNR â€” so
  ...
  // passenger) are left alone, and each ticket number stays independently editable â€”
  ...
  // customer's own recorded refund, if any â€” see the "Refunded ticket" select below.
  ...
  // ticket number â€” independent of whatever ticket the main form is currently
  ...
  // record) â€” those are grouped and merged in by customerIndex rather than one row
  ...
  // rows (e.g. when switching away from the refund option, or unchecking it) â€” leaving any
  ...
  // unless the main account has granted them permission to view all tickets â€” or granted
  ...
  // per-employee sections/ownership system above â€” it's reserved for the main account
  ...
  // whoever holds canManageYearLock (defined further down â€” Admin or GM/Owner-grade
  // only) â€” they keep seeing those records even while the year stays closed, since
  ...
  // If the current section is no longer (or was never) allowed for this employee â€”
  // e.g. their access was just changed by the main account â€” bounce them to the first
  ...
  useEffect(() => {
    if (!currentUser) return;
    // Analysis shares the same financially-sensitive gate as Accounts â€” it surfaces
  ...
  // employeeSectionPerm â€” falls back to their old account-wide toggles if a section was
  // never customized).
  ...
  // generic (non-section-scoped) helpers below â€” the flight ticket handlers, the
  // top-header badge, and the flight ticket detail page â€” keep using these names.
  ...
  // Accounting accounts can see everything but cannot add tickets â€” their only allowed
  ...
  // Every employee can add new tickets â€” this is no longer an individually
  ...
  // Manage employees and Backup/Restore, but never the License panel â€” that stays
  ...
  // manager") â€” distinct from the plain Accountant grade, which shares isAccounting but
  ...
  // Accounts Manager. Plain Accountant is deliberately excluded â€” closing/reopening a
  ...
  // or Accountant â€” see canAccessAccounts) still sees closed-year data across every
  ...
  // "Who can view/edit a closed year" picker inside the Closed years panel â€” stored per
  ...
  // if the line itself doesn't have dates â€” older bookings saved before dates lived on
  // each room line.
  ...
  // USD->EGP rate to use â€” normally the rate that was locked in on the record itself
  ...
  // Raw (un-converted, in the line's own currency) total for one line â€” used when showing
  ...
  // Car/transfer net, sold, and profit â€” net and sold can each be in a different
  ...
  // Flight ticket net/sold, converted to EGP â€” raw (pre-refund) figures, used for
  ...
  // Individual booking automatically â€” no separate toggle needed.
  const hotelBookingType = (h) => (h.customer && h.customer.trim() ? "Corporate" : "Individual");
  // A short readable summary of a booking's room lines, e.g. "1x Single (BB, 01-AUG-2026â†’05-AUG-2026), 2x Double (AI, 01-AUG-2026â†’03-AUG-2026)".
  ...
  const hotelLinesSummary = (h) =>
    (h.roomLines || [])
      .map((l) => {
        ...
        const dates = checkIn && checkOut ? `, ${formatDisplayDate(checkIn)}â†’${formatDisplayDate(checkOut)}` : "";
        return `${l.count}Ã— ${type} (${meal}${dates})`;
      })
      .join(", ");
  ...
  // Flights/Hotels/Files â€” falls back to matching on the display name for any legacy
  ...
  // see in the main Visa list â€” reusing the same permission-filtered list.
  ...
  // lists above â€” the accounting module always needs the full financial picture).
  ...
  const SECTION_LABELS_AR = { flights: "Ø·ÙŠØ±Ø§Ù†", hotels: "ÙÙ†Ø§Ø¯Ù‚", visa: "ÙÙŠØ²Ø§", cars: "ØªØ±Ø§Ù†Ø³ÙÙŠØ±" };
  ...
  const treasuryTransactions = [
    ...customerPayments.map((p) => ({
      ...
      label: accountsLang === "en" ? `Collection from ${p.customer || "-"}` : `ØªØ­ØµÙŠÙ„ Ù…Ù† ${p.customer || "-"}`, note: p.note, amount: parseFloat(p.amount) || 0,
    })),
    ...supplierPayments.map((p) => ({
      id: `sp-${p.id}`, date: p.date, accountId: p.accountId, direction: "out",
      label: accountsLang === "en" ? `Payment to ${p.supplier || "-"}` : `Ø¯ÙØ¹Ø© Ù„Ù€ ${p.supplier || "-"}`, note: p.note, amount: parseFloat(p.amount) || 0,
    })),
    ...expenses.map((e) => ({
  ...
  // carBookings/expenses from scratch on every render of this component â€” including renders
  ...
  // above) â€” they're safe to call in here without being listed as deps, since their only
  ...
  const handleSaveExpense = () => {
    if (!expenseForm.date || !expenseForm.category || expenseForm.amount === "" || !expenseForm.accountId) {
      setAccountsError("Ù…Ù† ÙØ¶Ù„Ùƒ Ø£ÙƒÙ…Ù„ Ø§Ù„ØªØ§Ø±ÙŠØ® ÙˆØ§Ù„ØªØµÙ†ÙŠÙ ÙˆØ§Ù„Ù…Ø¨Ù„Øº ÙˆØ§Ù„Ø®Ø²ÙŠÙ†Ø©/Ø§Ù„Ø­Ø³Ø§Ø¨");
      return;
    }
    setAccountsError("");
  ...
  const handleSaveTreasuryAccount = () => {
    if (!treasuryForm.name.trim()) {
      setAccountsError("Ù…Ù† ÙØ¶Ù„Ùƒ Ø§ÙƒØªØ¨ Ø§Ø³Ù… Ø§Ù„Ø­Ø³Ø§Ø¨/Ø§Ù„Ø®Ø²ÙŠÙ†Ø©");
      return;
    }
    setAccountsError("");
  ...
  const handleSaveTreasuryEntry = () => {
    if (!treasuryEntryForm.accountId || treasuryEntryForm.amount === "") {
      setAccountsError("Ù…Ù† ÙØ¶Ù„Ùƒ Ø§Ø®ØªØ± Ø§Ù„Ø­Ø³Ø§Ø¨ ÙˆØ§ÙƒØªØ¨ Ø§Ù„Ù…Ø¨Ù„Øº");
      return;
    }
    setAccountsError("");
  ...
  const handleSaveSupplierPayment = () => {
    if (!supplierPaymentForm.supplier || supplierPaymentForm.amount === "" || !supplierPaymentForm.accountId) {
      setAccountsError("Ù…Ù† ÙØ¶Ù„Ùƒ Ø§Ø®ØªØ± Ø§Ù„Ù…ÙˆØ±Ø¯ ÙˆØ§Ù„Ø®Ø²ÙŠÙ†Ø©/Ø§Ù„Ø­Ø³Ø§Ø¨ ÙˆØ§ÙƒØªØ¨ Ø§Ù„Ù…Ø¨Ù„Øº");
      return;
    }
    setAccountsError("");
  ...
  const handleSaveCustomerPayment = () => {
    if (!customerPaymentForm.customer || customerPaymentForm.amount === "" || !customerPaymentForm.accountId) {
      setAccountsError("Ù…Ù† ÙØ¶Ù„Ùƒ Ø§Ø®ØªØ± Ø§Ù„Ø¹Ù…ÙŠÙ„ ÙˆØ§Ù„Ø®Ø²ÙŠÙ†Ø©/Ø§Ù„Ø­Ø³Ø§Ø¨ ÙˆØ§ÙƒØªØ¨ Ø§Ù„Ù…Ø¨Ù„Øº");
      return;
    }
    setAccountsError("");
  ...
  // Exports the currently selected report range to an Excel workbook â€” revenue by
  // section, expenses by category, and the net profit summary â€” following the same
  // XLSX.utils/writeFile pattern used by the other sections' exports in this file.
  const handleExportAccountsReport = () => {
    const wb = XLSX.utils.book_new();
    const summaryRows = [
      { "Ø§Ù„Ø¨Ù†Ø¯": "Ø¥ÙŠØ±Ø§Ø¯Ø§Øª Ø§Ù„Ø·ÙŠØ±Ø§Ù†", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportRevenueBySection.flights * 100) / 100 },
      { "Ø§Ù„Ø¨Ù†Ø¯": "Ø¥ÙŠØ±Ø§Ø¯Ø§Øª Ø§Ù„ÙÙ†Ø§Ø¯Ù‚", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportRevenueBySection.hotels * 100) / 100 },
      { "Ø§Ù„Ø¨Ù†Ø¯": "Ø¥ÙŠØ±Ø§Ø¯Ø§Øª Ø§Ù„ÙÙŠØ²Ø§", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportRevenueBySection.visa * 100) / 100 },
      { "Ø§Ù„Ø¨Ù†Ø¯": "Ø¥ÙŠØ±Ø§Ø¯Ø§Øª Ø§Ù„ØªØ±Ø§Ù†Ø³ÙÙŠØ±", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportRevenueBySection.cars * 100) / 100 },
      { "Ø§Ù„Ø¨Ù†Ø¯": "Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportTotalRevenue * 100) / 100 },
      { "Ø§Ù„Ø¨Ù†Ø¯": "Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ù…ØµØ±ÙˆÙØ§Øª", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportTotalExpenses * 100) / 100 },
      { "Ø§Ù„Ø¨Ù†Ø¯": "ØµØ§ÙÙŠ Ø§Ù„Ø±Ø¨Ø­", "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(reportNetProfit * 100) / 100 },
    ];
    const summarySheet = XLSX.utils.json_to_sheet(summaryRows);
    XLSX.utils.book_append_sheet(wb, summarySheet, "Summary");
    const expenseRows = Object.entries(reportExpensesByCategory).map(([cat, amt]) => ({
      "Ø§Ù„ØªØµÙ†ÙŠÙ": cat,
      "Ø§Ù„Ù…Ø¨Ù„Øº (Ø¬.Ù…)": Math.round(amt * 100) / 100,
    }));
    ...
    const supplierRows = supplierLedger.map((s) => ({
      "Ø§Ù„Ù…ÙˆØ±Ø¯": s.supplier,
      "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ­Ù‚": Math.round(s.totalOwed * 100) / 100,
      "Ø§Ù„Ù…Ø¯ÙÙˆØ¹": Math.round(s.paid * 100) / 100,
      "Ø§Ù„Ù…ØªØ¨Ù‚Ù‰": Math.round(s.balance * 100) / 100,
    }));
    ...
    const customerRows = customerLedger.map((c) => ({
      "Ø§Ù„Ø¹Ù…ÙŠÙ„": c.customer,
      "Ø¥Ø¬Ù…Ø§Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ­Ù‚": Math.round(c.totalDue * 100) / 100,
      "Ø§Ù„Ù…Ø­ØµÙ„": Math.round(c.paid * 100) / 100,
      "Ø§Ù„Ù…ØªØ¨Ù‚Ù‰": Math.round(c.balance * 100) / 100,
    }));
  ...
  const visibleFiles = (
    ...
    // for same-day files â€” the list always follows the dates rather than raw creation/
    // array order.
    .slice()
  ...
  // (defaults to today, but the date is user-editable â€” see updateFileDate below):
  ...
  // points to (sourceType + sourceId). No price/label/date is duplicated here â€” that's
  ...
  // actual Flights/Hotels/Visa/Transportation record it's linked to â€” this is the single
  ...
  const resolveFileItem = (it) => {
    ...
    if (it.sourceType === "flights") {
      ...
      return {
        label: `${routeLabel(record)}${names ? " Â· " + names : ""}`,
        date: record.date,
    ...
    if (it.sourceType === "hotels") {
      ...
      if (!record) return missing;
      return {
        label: `${record.hotel || "Hotel"}${record.customer ? " Â· " + record.customer : ""}`,
    ...
    if (it.sourceType === "visa") {
      ...
      return {
        label: `${record.visaType || "Visa"}${names ? " Â· " + names : ""}`,
        date: record.bookingDate,
    ...
    if (it.sourceType === "cars") {
      ...
      if (!record) return missing;
      return {
        label: `${record.routeFrom || "-"} â†’ ${record.routeTo || "-"}${record.customerName ? " Â· " + record.customerName : ""}`,
        date: record.bookingDate,
        netCurrency: record.netCurrency || record.currency || "EGP",
        soldCurrency: record.soldCurrency || record.currency || "EGP",
        netPrice: parseFloat(record.netPrice) || 0,
  ...
  // record up via the item's stored sourceId â€” but WITHOUT switching activeSection,
  ...
  // was since deleted, there's nothing to jump to â€” surface a toast instead.
  //
  // `context` identifies where this item lives â€” { fileId, itemId } for a saved file,
  // or { draft: true, itemId } for the unsaved draft-file view â€” so the modal's Delete
  ...
  // signed-in employee (not gated by canDeleteTickets) â€” same reasoning as adding items to
  ...
  // never touched â€” the file will always show its current price live).
  ...
  // (not multiplied by customer count) â€” reducing sales by what went back to the customer
  ...
  // placed by ITS OWN date rather than always directly under the parent ticket â€”
  // same as the main table â€” and reuses the Net price / Sold price columns instead
  ...
  // and is NOT forced negative â€” it can land either side depending on the numbers.
  const ticketRows = (rows) => {
    ...
    const entries = sorted.flatMap((t) => {
      ...
      const ticketEntries = customers.map((c, i) => ({
        ...
        row: {
          ...
          // Net price / Sold price / Profit, and the totals row below â€” a
          // plain sum of this column â€” adds them up exactly as displayed.
          ...
          // EGP-converted figures (via the shared USD -> EGP rate) â€” these are what the
      ...
      const refundEntries = bookingRefunds.map((refund, ri) => {
        ...
        return {
          ...
          row: {
            ...
            "Ticket #": `Refund â€” ${(refundedCustomer && refundedCustomer.ticketNumber) || firstTicketNumber(t) || "ticket"}`,
            "Airline": airlineCode,
            "Route": routeLabel(t),
    ...
    // Places every row â€” refunds included â€” by its own date, so a refund lands
  ...
  // sheet rows â€” a plain column sum, so the totals row always matches exactly
  ...
  // are left blank on the totals row since they can mix currencies â€” only the
  ...
  const describeFilters = ({ month, year, company, employee, supplier, search } = {}) => {
    ...
    // (from the multi-select filters) â€” join arrays with an Arabic-style comma.
    const joinVal = (v) => (Array.isArray(v) ? v.join("ØŒ ") : v);
    const hasVal = (v) => (Array.isArray(v) ? v.length > 0 : !!v);
    const parts = [];
    if (hasVal(year)) parts.push(`Ø§Ù„Ø³Ù†Ø©: ${joinVal(year)}`);
    if (hasVal(month)) parts.push(`Ø§Ù„Ø´Ù‡Ø±: ${Array.isArray(month) ? month.map(monthLabel).join("ØŒ ") : monthLabel(month)}`);
    if (hasVal(company)) parts.push(`Ø§Ù„Ø´Ø±ÙƒØ©: ${joinVal(company)}`);
    if (hasVal(employee)) parts.push(`Ø§Ù„Ù…ÙˆØ¸Ù: ${joinVal(employee)}`);
    if (hasVal(supplier)) parts.push(`Ø§Ù„Ù…ÙˆØ±Ø¯: ${joinVal(supplier)}`);
    if (search) parts.push(`Ø¨Ø­Ø«: ${search}`);
    return parts.length ? `Ø§Ù„ÙÙ„Ø§ØªØ± Ø§Ù„Ù…Ø·Ø¨Ù‚Ø© â€” ${parts.join("  |  ")}` : "Ø¨Ø¯ÙˆÙ† ÙÙ„Ø§ØªØ± â€” ÙƒÙ„ Ø§Ù„Ù†ØªØ§Ø¦Ø¬";
  };

  // Writes rows to a sheet with a filter-summary banner merged across one cell at the
  // very top (row 1), a blank spacer row, then the normal header + data rows below â€”
  ...
  const sheetWithFilterBanner = (rows, filterLabel) => {
    ...
    const HEADER_ROW = 2; // 0-based row index â€” Excel row 3, right under the banner + spacer
    ...
    // numeric columns â€” SUBTOTAL(109, ...) automatically recalculates to only include
  ...
  // employee / supplier filters AND the search box (any combination) â€” the same set of
  // tickets currently shown on screen â€” sorted by issue date (same-day tickets ordered
  ...
  // Resets every filter and the search box at once â€” used by the "Clear all" action
  ...
  // Small "â‰ˆ X EGP" hint shown next to a price field whenever its currency is USD,
      ...
      <div className="w-full min-h-screen bg-gradient-to-br from-teal-50 via-stone-50 to-white flex items-center justify-center p-4">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 w-full max-w-sm shadow-xl shadow-teal-900/5" style={{ animation: "pdmPopIn .3s cubic-bezier(0.16,1,0.3,1) both" }}>
          ...
          <p className="text-xs text-stone-500 mb-4">
            No employees exist yet. Create the first account â€” it will be the main account, and only it will be able to add or remove other employees.
          </p>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <input type="password" className="w-full border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700"
                value={setupPassword} onChange={(e) => setSetupPassword(e.target.value)} placeholder="â€¢â€¢â€¢â€¢â€¢â€¢" />
            </div>
          </div>
          ...
          <p className="text-xs text-stone-400 mt-4">
            Note: this is a simple access gate stored with the app's data, not a secure authentication system â€” anyone with technical access to the app's data can read stored passwords. Don't reuse an important password here.
          </p>
        </div>
      </div>
    );
        ...
        <div className="relative w-full max-w-sm" style={{ animation: "pdmFadeIn .4s ease-out both" }}>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl shadow-black/30 overflow-hidden" style={{ animation: "pdmPopIn .45s cubic-bezier(0.16,1,0.3,1) .1s both" }}>
            ...
            <div className="relative bg-white px-6 pt-7 pb-6">
              {pendingTotpLogin ? (
                <>
                  {otpError && <div className="bg-red-50 text-red-700 text-sm rounded-2xl px-3 py-2 mb-3">{otpError}</div>}
                  <div className="text-center mb-4">
                    <div className="mx-auto w-11 h-11 rounded-full bg-teal-50 flex items-center justify-center mb-2">
                      <Smartphone size={20} className="text-teal-800" />
                    </div>
                    <p className="text-sm font-semibold text-stone-800">Two-factor authentication</p>
                    <p className="text-xs text-stone-400 mt-1">
                      Enter the 6-digit code from {pendingTotpLogin.match.name}'s authenticator app
                    </p>
                  </div>
                  <input
                    className="w-full border border-stone-300 rounded-2xl px-3 py-2.5 text-center text-lg tracking-[0.5em] font-semibold focus:outline-none focus:ring-2 focus:ring-teal-800 focus:border-teal-800"
                    value={otpCodeInput}
                  ...
                  />
                  <button onClick={handleVerifyLoginOtp}
                  ...
                  <button onClick={cancelTotpLogin} className="w-full mt-2 text-xs text-stone-400 hover:text-stone-600 py-1">
                    Back
                  </button>
                </>
              ) : (
  ...
  // sub-row if a refund has been recorded on this ticket. Every row always shows â€”
  // clicking a ticket's "Exchanged" or "Refunded"/"â†³ Refund" badge just scrolls to and
  // briefly highlights (green) the related row elsewhere in the table.
  // Each entry is tagged with its own sortDate so the caller can place every row â€”
  // including refund rows â€” by that row's own date, instead of always nesting the
        ...
        >
          ...
          <td className={`px-1 py-0 font-medium ${nameText} whitespace-nowrap`}>
            <span className="inline-flex items-center gap-1.5">
              ...
              {t.voided && i === 0 && (
                <span
                  title="Voided â€” excluded from totals"
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {t.isReissued && (
                <span
                  ...
                  title={`Exchanged from ${t.oldTicketNumber || "an older ticket"} â€” click to view the original ticket`}
              ...
              {refundForIndex(t, i) && (
                <span
                  ...
                  title={`Refunded â€” Airline: ${fmt(refundForIndex(t, i).airlineAmount)} Â· Customer: ${fmt(refundForIndex(t, i).customerAmount)} â€” click to view the refund`}
          ...
          <td className={`px-1 py-0 ${rowText} font-mono whitespace-nowrap`}>
            <span className="inline-flex items-center gap-1.5">
              ...
              <span
                ...
                title="Refund â€” click to view the original ticket"
    ...
    >
        ...
        <div className="fixed inset-0 z-[100] bg-gradient-to-br from-teal-900 via-teal-800 to-[#0d3b3e] flex items-center justify-center p-4">
          <div className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl shadow-black/30 overflow-hidden" style={{ animation: "pdmPopIn .3s cubic-bezier(0.16,1,0.3,1) both" }}>
            <div className="relative bg-gradient-to-r from-teal-800 to-teal-900 px-6 pt-8 pb-7 text-center">
              ...
              <p className="text-teal-50/90 text-xs mt-1">Signed in as {currentUser.name} â€” enter your password to continue</p>
            </div>
      ...
      <div className="relative max-w-5xl mx-auto xl:mx-0 xl:w-[64rem] xl:shrink-0 p-4 md:p-6">
        ...
        <div className="relative rounded-2xl bg-gradient-to-r from-teal-800 via-teal-800 to-teal-900 shadow-lg shadow-teal-900/20 mb-0">
          ...
          <header className="relative flex flex-col gap-2 sm:gap-3 px-3 py-2.5 sm:px-4 sm:py-3 md:px-6 md:py-4">
            ...
            <div className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-2 z-10 order-2 max-w-full">
              ...
              {/* "Management" flyout â€” groups the account/admin icon actions behind one
                  ...
                  all â€” everyone else, even with Corporates/Suppliers access, doesn't. */}
              {hasAdminAccess && (
              <div className="relative">
                <button
                  type="button"
            ...
            <div className="bg-white rounded-2xl border border-stone-200 p-4 md:p-5 w-full max-w-sm my-8 md:my-0 max-h-[90vh] overflow-y-auto">
              ...
              {isLicensed ? (
                <div className="bg-emerald-50 text-emerald-700 text-sm rounded-xl px-3 py-2 mb-4 mt-3">
                  Active{licenseRecord && licenseRecord.expiresAt ? ` â€” valid until ${licenseRecord.expiresAt}` : " â€” permanent license"}
                </div>
              ) : (
                <p className="text-xs text-stone-400 mb-4 mt-3">
                  {licenseRecord ? "The current activation code has expired." : "The app is not activated yet."} Enter a valid activation code below â€” the app stays locked for every employee until this is done.
                </p>
              )}
              {isLicensed && (
                <p className="text-xs text-stone-400 mb-4">
                  Entering a new code below will replace the current one â€” useful for renewing or switching licenses.
                </p>
              )}
        ...
        {showClosedYearsPanel && canManageYearLock && (() => {
          ...
          // Every year that has at least one dated record in ANY section â€” the year
            ...
            >
              <div className="bg-white rounded-2xl border border-stone-200 p-6 md:p-8 w-full max-w-4xl my-8 md:my-0 max-h-[90vh] overflow-y-auto">
                ...
                <p className="text-sm text-stone-400 mb-6 mt-3">
                  A closed year disappears completely for every employee â€” from lists, filters, stats, and exports â€” in
                ...
                <div className="border-t border-stone-200 mt-6 pt-5">
                  ...
                  <p className="text-xs text-stone-400 mb-4">
                    Admin and Owner/GM can always see and act on every closed year â€” they're the ones who can
                        ...
                        {(employees || [])
                          ...
                          .map((e) => (
                            <option key={e.username} value={e.username}>
                              {(e.name || e.username)} (@{e.username} Â· {roleLabel(e.role)})
                            </option>
                          ))}
                      </select>
                    </div>
                    ...
                    {[...filteredHistory].sort((a, b) => (b.at || 0) - (a.at || 0)).map((entry, idx) => (
                      ...
                      >
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <p className="text-xs text-stone-400 whitespace-nowrap mt-0.5">{entry.at ? formatDateTime(entry.at) : "â€”"}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 p-4 md:p-5 mb-6">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-stone-500 whitespace-nowrap">
                          <span className="font-mono" title="Passwords are stored securely and can't be viewed â€” click the employee's name to set a new one">â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢</span>
                        </td>
                        <td className="px-3 py-2 text-stone-500 whitespace-nowrap">
                          {e.isAdmin ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-teal-800 bg-teal-50 border border-teal-200 rounded-full px-2 py-0.5">
                        ...
                        <td className="px-3 py-2 text-right whitespace-nowrap">
                          <div className="flex gap-1 justify-end">
                            {/* Promoting/demoting main-account access stays reserved for true
                                main accounts â€” an Owner never gets to touch this, so an Owner
            ...
            <div className="border border-stone-200 rounded-2xl p-4 bg-stone-50">
              ...
              {/* Grade picker: one dropdown per department (Flights / Hotels / Visa /
                  ...
                  Name/username/password above are never touched by any of this â€” they
                ...
                <div className="space-y-1.5 mb-2">
                  {newCompanyDraft.deals.map((d, i) => (
                    <div key={i} className="flex items-center justify-between bg-stone-100 rounded-xl px-3 py-1.5 text-sm">
                      <span>
                        {d.airline && <span className="font-semibold text-stone-700">{d.airline.toUpperCase()}{" â€” "}</span>}
                                  ...
                                  {deals.length > 0
                                    ? deals.map((d, i) => (
                                        <span key={i} className="block whitespace-nowrap">
                                          {d.airline ? `${d.airline.toUpperCase()} â€” ${d.details}` : d.details}
                                        </span>
                                      ))
                                    : "-"}
                                </td>
        ...
        {/* USD -> EGP exchange rate bar â€” shown once above every section (not just
        ...
        <div className="bg-white border border-stone-200 rounded-2xl px-4 py-3 mb-4 flex flex-wrap items-center gap-3">
          <span className="text-xs font-semibold text-stone-500">USD â†’ EGP rate today:</span>
          <input
            type="number"
            step="0.01"
          ...
          {usdToEgpRateDate && (() => {
            ...
            return (
              <span
                ...
                title={isStale ? "This rate wasn't updated today â€” double-check before relying on it" : "Rate updated today"}
              >
                <Clock size={12} />
        ...
        {activeSection === "flights" && (
          <FlightsSection
            ...
            HOTEL_CURRENCIES={HOTEL_CURRENCIES}
            NET_PAYMENT_METHODS={NET_PAYMENT_METHODS}
            ThFilter={ThFilter}
            addCentsOnBlur={addCentsOnBlur}
            addDestinationStop={addDestinationStop}
            addRefundRow={addRefundRow}
            airlinesAvailable={airlinesAvailable}
            applyIataTicketValue={applyIataTicketValue}
            canAddTickets={canAddTickets}
            canEditTickets={canEditTickets}
            clearAllFilters={clearAllFilters}
            clearAllRefundRows={clearAllRefundRows}
            companiesAvailable={companiesAvailable}
            companyBreakdown={companyBreakdown}
            companyDeals={companyDeals}
            companyName={companyName}
            copiedDealIndex={copiedDealIndex}
            corporateDropdownOpen={corporateDropdownOpen}
            corporateDropdownRef={corporateDropdownRef}
            currentMonthKey={currentMonthKey}
            currentUser={currentUser}
            dealsDropdownOpen={dealsDropdownOpen}
            dealsDropdownRef={dealsDropdownRef}
            employeesAvailable={employeesAvailable}
            exportFiltered={exportFiltered}
            exportMonth={exportMonth}
            findTicketByNumber={findTicketByNumber}
            flightApiKey={flightApiKey}
            flightApiKeyDraft={flightApiKeyDraft}
            flightLookupError={flightLookupError}
            flightLookupLoading={flightLookupLoading}
            flightLookupNumber={flightLookupNumber}
            ...
            handleClearFlightApiKey={handleClearFlightApiKey}
            handleCustomerConjunctionToggle={handleCustomerConjunctionToggle}
            handleCustomerFieldChange={handleCustomerFieldChange}
            handleCustomersCountChange={handleCustomersCountChange}
            handleCustomerTypeChange={handleCustomerTypeChange}
            handleDestinationChange={handleDestinationChange}
            handleFormFlightLookup={handleFormFlightLookup}
            handleOldTicketNumberBlur={handleOldTicketNumberBlur}
            handleOldTicketNumberChange={handleOldTicketNumberChange}
            handlePnrReferenceBlur={handlePnrReferenceBlur}
            handleRefundRowNumberBlur={handleRefundRowNumberBlur}
            handleRefundRowNumberChange={handleRefundRowNumberChange}
            handleSaveFlightApiKey={handleSaveFlightApiKey}
            handleSubmit={handleSubmit}
            handleTicketNumberBlur={handleTicketNumberBlur}
            handleTicketScreenshot={handleTicketScreenshot}
            hasActiveFilter={hasActiveFilter}
            iataBalance={iataBalance}
            iataHistory={iataHistory}
            iataTicketValueInput={iataTicketValueInput}
            isAccountingUser={isAccountingUser}
            lookupFlight={lookupFlight}
            monthLabel={monthLabel}
            monthlyBreakdown={monthlyBreakdown}
            monthsAvailable={monthsAvailable}
            multiFilterGroup={multiFilterGroup}
            MultiSelectDropdown={MultiSelectDropdown}
            persistIataBalance={persistIataBalance}
            query={query}
            refundBoxOpen={refundBoxOpen}
            refundRows={refundRows}
            refundSaved={refundSaved}
            removeDestinationStop={removeDestinationStop}
            removeRefundRow={removeRefundRow}
            restoreError={restoreError}
            restoreSuccess={restoreSuccess}
            routeLabel={routeLabel}
            saveAllRefunds={saveAllRefunds}
            ...
            setCopiedDealIndex={setCopiedDealIndex}
            setCorporateDropdownOpen={setCorporateDropdownOpen}
            setDealsDropdownOpen={setDealsDropdownOpen}
            setFiltersOpen={setFiltersOpen}
            setFlightApiKeyDraft={setFlightApiKeyDraft}
            setFlightLookupNumber={setFlightLookupNumber}
            setForm={setForm}
            setIataBalance={setIataBalance}
            setIataTicketValueInput={setIataTicketValueInput}
            setQuery={setQuery}
            setRefundBoxOpen={setRefundBoxOpen}
            setRefundRows={setRefundRows}
            setRefundSaved={setRefundSaved}
            ...
            setShowFlightLookup={setShowFlightLookup}
            setShowIataHistory={setShowIataHistory}
            setSupplierOther={setSupplierOther}
            showFlightLookup={showFlightLookup}
            showIataHistory={showIataHistory}
            ...
            ticketProfitEgp={ticketProfitEgp}
            ticketScanError={ticketScanError}
            ticketScanLoading={ticketScanLoading}
        ...
        {activeSection === "hotels" && (
          <HotelsSection
            ...
            fmt={fmt}
            formatDisplayDate={formatDisplayDate}
            hasActiveHotelFilter={hasActiveHotelFilter}
            hotelCurrentMonthTotals={hotelCurrentMonthTotals}
            activeHotelFilterCount={activeHotelFilterCount}
            hotelDateRange={hotelDateRange}
            hotelEditingId={hotelEditingId}
            hotelError={hotelError}
            hotelFiltersOpen={hotelFiltersOpen}
            hotelForm={hotelForm}
            hotelInEgp={hotelInEgp}
            hotelNameOther={hotelNameOther}
            hotelsPerm={hotelsPerm}
            hotelQuery={hotelQuery}
            hotelSelectedEmployee={hotelSelectedEmployee}
            hotelSelectedHotelName={hotelSelectedHotelName}
            hotelSelectedMonth={hotelSelectedMonth}
            hotelSelectedSupplier={hotelSelectedSupplier}
            hotelSelectedYear={hotelSelectedYear}
            hotelSupplierOther={hotelSupplierOther}
            hotelTotals={hotelTotals}
            hotelYearsAvailable={hotelYearsAvailable}
            hotelMonthsAvailable={hotelMonthsAvailable}
            hotelEmployeesAvailable={hotelEmployeesAvailable}
            hotelSuppliersAvailable={hotelSuppliersAvailable}
            hotelNamesAvailable={hotelNamesAvailable}
            ...
            monthLabel={monthLabel}
            multiFilterGroup={multiFilterGroup}
            newHotelNameDraft={newHotelNameDraft}
            newSupplierDraft={newSupplierDraft}
            navigateToSection={navigateToSection}
            rankByServiceDate={rankByServiceDate}
            ...
            setCopyPickerSource={setCopyPickerSource}
            setHotelFiltersOpen={setHotelFiltersOpen}
            setHotelForm={setHotelForm}
            setHotelNameOther={setHotelNameOther}
            setHotelQuery={setHotelQuery}
            setHotelSupplierOther={setHotelSupplierOther}
            setNewHotelNameDraft={setNewHotelNameDraft}
            setNewSupplierDraft={setNewSupplierDraft}
            setShowAddHotelNamePanel={setShowAddHotelNamePanel}
            setShowAddSupplierPanel={setShowAddSupplierPanel}
            setViewingFileContext={setViewingFileContext}
            setViewingHotelBooking={setViewingHotelBooking}
            showAddHotelNamePanel={showAddHotelNamePanel}
            showAddSupplierPanel={showAddSupplierPanel}
            suggestions={suggestions}
            updateHotelRoomLine={updateHotelRoomLine}
            updateRoomChild={updateRoomChild}
            updateRoomGuest={updateRoomGuest}
            clearAllHotelFilters={clearAllHotelFilters}
            handleSaveHotel={handleSaveHotel}
            resetHotelForm={resetHotelForm}
            sanitizeAgeInput={sanitizeAgeInput}
            usdHint={usdHint}
            usdToEgpRate={usdToEgpRate}
            viewingHotelBooking={viewingHotelBooking}
            visibleHotelBookings={visibleHotelBookings}
            handleAddHotelName={handleAddHotelName}
            handleAddSupplierName={handleAddSupplierName}
            handleDeleteHotel={handleDeleteHotel}
            handleDeleteHotelName={handleDeleteHotelName}
            handleDeleteSupplierName={handleDeleteSupplierName}
            handleDuplicateHotelClick={handleDuplicateHotelClick}
            ...
            hotelLinesSummary={hotelLinesSummary}
          ...
          />
                ...
                {activeSection === "visa" && (
          <VisaSection
            hasActiveVisaFilter={hasActiveVisaFilter} visaSelectedYear={visaSelectedYear} visaSelectedMonth={visaSelectedMonth}
            visaSelectedEmployee={visaSelectedEmployee} visaSelectedSupplier={visaSelectedSupplier} monthLabel={monthLabel}
            currentMonthKey={currentMonthKey} PassportIcon={PassportIcon} visaTotals={visaTotals}
            visaCurrentMonthTotals={visaCurrentMonthTotals} fmt={fmt} setShowAddVisaSupplierPanel={setShowAddVisaSupplierPanel}
            showAddVisaSupplierPanel={showAddVisaSupplierPanel} setShowVisaChecker={setShowVisaChecker} showVisaChecker={showVisaChecker}
            visaApiKey={visaApiKey} visaApiKeyDraft={visaApiKeyDraft} setVisaApiKeyDraft={setVisaApiKeyDraft}
            handleSaveVisaApiKey={handleSaveVisaApiKey} visaCheckPassport={visaCheckPassport} setVisaCheckPassport={setVisaCheckPassport}
            VISA_COUNTRY_LIST={VISA_COUNTRY_LIST} visaCheckDestination={visaCheckDestination}
            setVisaCheckDestination={setVisaCheckDestination} visaCheckLoading={visaCheckLoading}
            checkVisaRequirement={checkVisaRequirement} currentUser={currentUser} handleClearVisaApiKey={handleClearVisaApiKey}
            visaCheckError={visaCheckError} visaCheckResult={visaCheckResult} VISA_RULE_COLOR_CLASSES={VISA_RULE_COLOR_CLASSES}
            suggestions={suggestions} newVisaSupplierDraft={newVisaSupplierDraft} setNewVisaSupplierDraft={setNewVisaSupplierDraft}
            handleAddVisaSupplierName={handleAddVisaSupplierName} handleDeleteVisaSupplierName={handleDeleteVisaSupplierName}
            visaError={visaError} visaEditingId={visaEditingId} visaForm={visaForm} setVisaForm={setVisaForm}
            companyName={companyName} visaSupplierOther={visaSupplierOther} setVisaSupplierOther={setVisaSupplierOther}
            HOTEL_CURRENCIES={HOTEL_CURRENCIES} addCentsOnBlur={addCentsOnBlur} usdHint={usdHint} visaPerm={visaPerm}
            filteredVisaBookings={filteredVisaBookings} visaQuery={visaQuery} setVisaQuery={setVisaQuery}
            visaFiltersOpen={visaFiltersOpen} setVisaFiltersOpen={setVisaFiltersOpen} visaYearsAvailable={visaYearsAvailable}
            visaMonthsAvailable={visaMonthsAvailable} visaEmployeesAvailable={visaEmployeesAvailable}
            visaSuppliersAvailable={visaSuppliersAvailable} visibleVisaBookings={visibleVisaBookings} setVisaSelectedYear={setVisaSelectedYear}
            setVisaSelectedMonth={setVisaSelectedMonth} setVisaSelectedEmployee={setVisaSelectedEmployee}
            setVisaSelectedSupplier={setVisaSelectedSupplier} MultiSelectDropdown={MultiSelectDropdown}
            AppliedFilters={AppliedFilters} multiFilterGroup={multiFilterGroup} clearAllVisaFilters={clearAllVisaFilters}
            ThFilter={ThFilter} formatDisplayDate={formatDisplayDate} handleVisaCustomerNameChange={handleVisaCustomerNameChange}
            handleVisaCustomersCountChange={handleVisaCustomersCountChange} handleSaveVisa={handleSaveVisa}
            handleEditVisaClick={handleEditVisaClick} handleDuplicateVisaClick={handleDuplicateVisaClick}
            handleDeleteVisa={handleDeleteVisa} handlePrintVisa={handlePrintVisa} resetVisaForm={resetVisaForm}
            viewingVisaBooking={viewingVisaBooking} setViewingVisaBooking={setViewingVisaBooking}
            visaNetTotal={visaNetTotal} visaSoldTotal={visaSoldTotal} visaProfitTotal={visaProfitTotal}
            usdToEgpRate={usdToEgpRate} setCopyPickerSource={setCopyPickerSource} setViewingFileContext={setViewingFileContext}
            viewingFileContext={viewingFileContext} removeDraftItem={removeDraftItem} removeItemFromFile={removeItemFromFile}
          />
        )}
{activeSection === "cars" && (
          <TransportationSection
            hasActiveCarFilter={hasActiveCarFilter} carSelectedYear={carSelectedYear} carSelectedMonth={carSelectedMonth}
            carSelectedSupplier={carSelectedSupplier} monthLabel={monthLabel} currentMonthKey={currentMonthKey}
            carTotals={carTotals} carCurrentMonthTotals={carCurrentMonthTotals} fmt={fmt}
            setShowAddCarSupplierPanel={setShowAddCarSupplierPanel} showAddCarSupplierPanel={showAddCarSupplierPanel}
            newCarSupplierDraft={newCarSupplierDraft} setNewCarSupplierDraft={setNewCarSupplierDraft}
            handleAddCarSupplierName={handleAddCarSupplierName} suggestions={suggestions}
            handleDeleteCarSupplierName={handleDeleteCarSupplierName} carError={carError} canAddTickets={canAddTickets}
            carEditingId={carEditingId} carForm={carForm} setCarForm={setCarForm} companyName={companyName}
            HOTEL_CURRENCIES={HOTEL_CURRENCIES} addCentsOnBlur={addCentsOnBlur} usdHint={usdHint} carPerm={carPerm}
            handleSaveCar={handleSaveCar} carQuery={carQuery} setCarQuery={setCarQuery}
            carFiltersOpen={carFiltersOpen} setCarFiltersOpen={setCarFiltersOpen}
            activeCarFilterCount={activeCarFilterCount} carYearsAvailable={carYearsAvailable}
            carMonthsAvailable={carMonthsAvailable} carSuppliersAvailable={carSuppliersAvailable}
            carSelectedEmployee={carSelectedEmployee} MultiSelectDropdown={MultiSelectDropdown}
            AppliedFilters={AppliedFilters} multiFilterGroup={multiFilterGroup} clearAllCarFilters={clearAllCarFilters}
            filteredCarBookings={filteredCarBookings} visibleCarBookings={visibleCarBookings}
            ThFilter={ThFilter} formatDisplayDate={formatDisplayDate}
            handleCarCustomerNameChange={handleCarCustomerNameChange} handleCarPassengersChange={handleCarPassengersChange}
            handleEditCarClick={handleEditCarClick} handleDuplicateCarClick={handleDuplicateCarClick}
            handleDeleteCar={handleDeleteCar} handlePrintCar={handlePrintCar} resetCarForm={resetCarForm}
            viewingCarBooking={viewingCarBooking} setViewingCarBooking={setViewingCarBooking}
            carNetTotal={carNetTotal} carSoldTotal={carSoldTotal} carProfitTotal={carProfitTotal}
            usdToEgpRate={usdToEgpRate} setCopyPickerSource={setCopyPickerSource}
            setViewingFileContext={setViewingFileContext} viewingFileContext={viewingFileContext}
            removeDraftItem={removeDraftItem} removeItemFromFile={removeItemFromFile}
            ExtractedTimeSelect={ExtractedTimeSelect} CAR_TYPES={CAR_TYPES} carSupplierOther={carSupplierOther}
            setCarSupplierOther={setCarSupplierOther} isYearLocked={isYearLocked} carsPerm={carsPerm}
            navigateToSection={navigateToSection}
          />
        )}        {activeSection === "activities" && <ExtractedActivitiesSection />}

        {activeSection === "files" && (
          <FilesSection
            fileError={fileError} openFile={openFile} draftFile={draftFile} visibleFiles={visibleFiles}
            filesGrandTotals={filesGrandTotals} fmt={fmt} fileQuery={fileQuery} setFileQuery={setFileQuery}
            fileFiltersOpen={fileFiltersOpen} setFileFiltersOpen={setFileFiltersOpen}
            activeFileFilterCount={activeFileFilterCount} fileYearsAvailable={fileYearsAvailable}
            fileSelectedYear={fileSelectedYear} setFileSelectedYear={setFileSelectedYear}
            fileCompaniesAvailable={fileCompaniesAvailable} fileSelectedCompany={fileSelectedCompany}
            setFileSelectedCompany={setFileSelectedCompany} fileEmployeesAvailable={fileEmployeesAvailable}
            fileSelectedEmployee={fileSelectedEmployee} setFileSelectedEmployee={setFileSelectedEmployee}
            MultiSelectDropdown={MultiSelectDropdown} AppliedFilters={AppliedFilters}
            multiFilterGroup={multiFilterGroup} clearAllFileFilters={clearAllFileFilters}
            startNewFileDraft={startNewFileDraft} filteredFiles={filteredFiles} fileTotals={fileTotals}
            isYearLocked={isYearLocked} setOpenFileId={setOpenFileId}
            setEditingFileServices={setEditingFileServices} filesPerm={filesPerm}
            requestConfirm={requestConfirm} formatDisplayDate={formatDisplayDate} deleteFile={deleteFile}
            setConfirmDialog={setConfirmDialog} cancelDraftFile={cancelDraftFile} todayDateStr={todayDateStr}
            updateDraftField={updateDraftField} updateDraftDate={updateDraftDate}
            showFilePicker={showFilePicker} setShowFilePicker={setShowFilePicker}
            confirmDraftFile={confirmDraftFile} resolveFileItem={resolveFileItem}
            FILE_SOURCE_LABELS={FILE_SOURCE_LABELS} viewFileItemDetails={viewFileItemDetails}
            removeDraftItem={removeDraftItem} handlePrintFile={handlePrintFile}
            updateFileDate={updateFileDate} updateFileField={updateFileField}
            editingFileServices={editingFileServices} suggestions={suggestions} companyName={companyName}
            removeItemFromFile={removeItemFromFile} filePickerTab={filePickerTab}
            setFilePickerTab={setFilePickerTab} PassportIcon={PassportIcon}
            visibleTickets={visibleTickets} addDraftItem={addDraftItem} addItemToFile={addItemToFile}
            routeLabel={routeLabel} getCustomers={getCustomers} visibleHotelBookings={visibleHotelBookings}
            hotelSoldTotal={hotelSoldTotal} visibleVisaBookingsForFiles={visibleVisaBookingsForFiles}
            visaSoldTotal={visaSoldTotal} visibleCarBookings={visibleCarBookings}
          />
        )}
        ...
        {activeSection === "analysis" && canAccessAccounts && (
          <AnalysisSection
            ...
            soldAfterRefund={soldAfterRefund}
            ...
            hotelProfitTotal={hotelProfitTotal}
            hotelInEgp={hotelInEgp}
            visaSoldTotal={visaSoldTotal}
            visaNetTotal={visaNetTotal}
            visaProfitTotal={visaProfitTotal}
            carSoldTotal={carSoldTotal}
            carNetTotal={carNetTotal}
            carProfitTotal={carProfitTotal}
            todayDateStr={todayDateStr}
            empSalesRange={empSalesRange}
            setEmpSalesRange={setEmpSalesRange}
            empSalesFrom={empSalesFrom}
            setEmpSalesFrom={setEmpSalesFrom}
            empSalesTo={empSalesTo}
            setEmpSalesTo={setEmpSalesTo}
            fmt={fmt}
            BarChart3={BarChart3}
          />
        )}
        ...
        {activeSection === "accounts" && canAccessAccounts && (
          <AccountsSection
            ...
            filteredSupplierLedger={filteredSupplierLedger}
            setViewingSupplier={setViewingSupplier}
            accountsLang={accountsLang}
            customerQuery={customerQuery}
            setCustomerQuery={setCustomerQuery}
            filteredCustomerLedger={filteredCustomerLedger}
            setViewingCustomer={setViewingCustomer}
            setTreasuryForm={setTreasuryForm}
            getEmptyTreasuryAccountForm={getEmptyTreasuryAccountForm}
            setTreasuryAccountEditingId={setTreasuryAccountEditingId}
            setShowTreasuryAccountForm={setShowTreasuryAccountForm}
            treasuryAccounts={treasuryAccounts}
            handleEditTreasuryAccountClick={handleEditTreasuryAccountClick}
            handleDeleteTreasuryAccount={handleDeleteTreasuryAccount}
            treasuryAccountTypeLabel={treasuryAccountTypeLabel}
            treasuryBalance={treasuryBalance}
            treasuryFilterAccountId={treasuryFilterAccountId}
            setTreasuryFilterAccountId={setTreasuryFilterAccountId}
            setTreasuryEntryForm={setTreasuryEntryForm}
            getEmptyTreasuryEntryForm={getEmptyTreasuryEntryForm}
            setShowTreasuryEntryForm={setShowTreasuryEntryForm}
            filteredTreasuryTransactions={filteredTreasuryTransactions}
            formatDisplayDate={formatDisplayDate}
            handleDeleteTreasuryEntry={handleDeleteTreasuryEntry}
            handleDeleteSupplierPayment={handleDeleteSupplierPayment}
            ...
            getEmptyExpenseForm={getEmptyExpenseForm}
            setExpenseEditingId={setExpenseEditingId}
            ...
            setReportsFrom={setReportsFrom}
            reportsTo={reportsTo}
            setReportsTo={setReportsTo}
            handleExportAccountsReport={handleExportAccountsReport}
            reportRevenueBySection={reportRevenueBySection}
            reportBookingsCount={reportBookingsCount}
          ...
          <div className="max-w-3xl mx-auto p-4 md:p-6">
            ...
            <div className="bg-white border border-stone-200 rounded-2xl divide-y divide-stone-100">
              {viewingTicket.voided && (
                <div className="bg-stone-100 border-b border-stone-200 px-4 md:px-5 py-2.5 flex items-center gap-1.5 text-stone-600 rounded-t-2xl">
                  ...
                  <p className="text-xs font-semibold">
                    This ticket is voided â€” its amounts are excluded from totals
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 p-4 md:p-5">
                <div>
                  ...
                  {viewingTicket.netCurrency === "USD" && (viewingTicket.usdRate ?? usdToEgpRate) && (
                    <p className="text-[11px] text-emerald-600">
                      â‰ˆ {fmt(ticketNetTotal(viewingTicket) * (viewingTicket.usdRate ?? usdToEgpRate))} EGP Â· rate {fmt(viewingTicket.usdRate ?? usdToEgpRate)}
                    </p>
                  )}
                  {(ticketPaxCounts(viewingTicket).child > 0 || ticketPaxCounts(viewingTicket).infant > 0) && (
                    <p className="text-[11px] text-stone-400">
                      Adult {fmt(viewingTicket.netPrice)}
                      {ticketPaxCounts(viewingTicket).child > 0 && ` Â· Child ${fmt(viewingTicket.childNetPrice)} Ã— ${ticketPaxCounts(viewingTicket).child}`}
                      {ticketPaxCounts(viewingTicket).infant > 0 && ` Â· Infant ${fmt(viewingTicket.infantNetPrice)} Ã— ${ticketPaxCounts(viewingTicket).infant}`}
                    </p>
                  )}
                ...
                <div>
                  ...
                  {viewingTicket.soldCurrency === "USD" && (viewingTicket.usdRate ?? usdToEgpRate) && (
                    <p className="text-[11px] text-emerald-600">
                      â‰ˆ {fmt(ticketSoldTotal(viewingTicket) * (viewingTicket.usdRate ?? usdToEgpRate))} EGP Â· rate {fmt(viewingTicket.usdRate ?? usdToEgpRate)}
                    </p>
                  )}
                  {(ticketPaxCounts(viewingTicket).child > 0 || ticketPaxCounts(viewingTicket).infant > 0) && (
                    <p className="text-[11px] text-stone-400">
                      Adult {fmt(viewingTicket.soldPrice)}
                      {ticketPaxCounts(viewingTicket).child > 0 && ` Â· Child ${fmt(viewingTicket.childSoldPrice)} Ã— ${ticketPaxCounts(viewingTicket).child}`}
                      {ticketPaxCounts(viewingTicket).infant > 0 && ` Â· Infant ${fmt(viewingTicket.infantSoldPrice)} Ã— ${ticketPaxCounts(viewingTicket).infant}`}
                    </p>
                  )}
                  {hasRefund(viewingTicket) && (
              ...
              {/* Refund(s): entered via the checkbox box in the main ticket form (next to
                  Reissue), so this is a read-only summary â€” edit it by editing the
                          ...
                          <span className="w-full">
                            ...
                            <span className="text-sky-900 font-medium">
                              {(() => {
                                ...
                                return c ? (c.name || `Customer ${idx + 1}`) + (c.ticketNumber ? ` â€” ${c.ticketNumber}` : "") : `Customer ${idx + 1}`;
                              })()}
                            </span>
                          </span>
                        )}
              ...
              <div className="p-4 md:p-5">
                ...
                <textarea
                  ...
                  placeholder="No notes yet â€” add some here"
                />
      ...
      {/* Incoming-request notification popup â€” floats in the corner without blocking the
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-1">
              ...
              <button title="Close" onClick={closeSecurityModal} className="text-stone-400 hover:text-stone-700 p-1">
                <X size={16} />
              </button>
            </div>
            <p className="text-xs text-stone-400 mb-4">
            ...
            <div className="mt-5 pt-4 border-t border-stone-200">
              <h3 className="font-semibold text-stone-900 flex items-center gap-2 text-sm mb-1">
                <Smartphone size={15} className="text-teal-800" /> Two-factor authentication
              </h3>
              {totpSetupSuccess && <div className="bg-emerald-50 text-emerald-700 text-xs rounded-xl px-3 py-2 mb-2 mt-2">{totpSetupSuccess}</div>}
              {(() => {
                const me = (employees || []).find((e) => e.username === currentUser.username);
                    ...
                    <div className="mt-2 space-y-3">
                      ...
                      <div className="flex justify-center">
                        {totpSetupQr ? (
                          ...
                          <div className="w-40 h-40 rounded-lg border border-stone-200 flex items-center justify-center text-stone-300 text-xs">Generatingâ€¦</div>
                        )}
                      </div>
                  ...
                  return (
                    <div className="mt-2 space-y-2">
                      <p className="text-xs text-emerald-700 flex items-center gap-1">
                        <ShieldCheck size={13} /> Enabled â€” you'll be asked for a code from your app when you sign in.
                      </p>
                      {totpDisableError && <div className="bg-red-50 text-red-700 text-xs rounded-xl px-3 py-2">{totpDisableError}</div>}
                      <div className="flex gap-2">
                        <input type="password"
                          className="flex-1 border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700"
                          value={totpDisablePassword} onChange={(e) => setTotpDisablePassword(e.target.value)}
                          placeholder="Confirm password to disable" />
                        <button onClick={disableTotp}
                          className="px-3 py-2 text-xs font-semibold rounded-xl border border-red-300 text-red-600 hover:bg-red-50 shrink-0">
                          Disable
                        </button>
                      </div>
                    </div>
                ...
                return (
                  <div className="mt-2">
                    <p className="text-xs text-stone-400 mb-2">Off â€” add an authenticator app for an extra step at sign-in.</p>
                    <button onClick={startTotpSetup}
                      className="w-full bg-white border border-teal-800 text-teal-800 hover:bg-teal-50 text-xs font-semibold rounded-xl px-3 py-2">
                      Set up 2FA
                    </button>
                  </div>
                );
      ...
      {showIataHistory && (
        <div className="fixed inset-0 bg-stone-900/40 flex items-center justify-center p-4 z-50" onClick={() => setShowIataHistory(false)}>
          ...
          >
            ...
            <p className="text-xs text-stone-400 mb-3">Today only â€” resets empty at the start of each day</p>
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-stone-400 mb-3">
              Links this {FILE_SOURCE_LABELS[copyPickerSource.type] || copyPickerSource.type} record into the file â€” its price stays live, and the original record is never touched.
            </p>
                ...
                return filteredFiles.map((f) => (
                  ...
                  >
                    <span className="truncate">
                      {f.serial} {f.company ? `Â· ${f.company}` : ""}
                    </span>
      ...
      {/* Print preview popup â€” used by every service's Print button. Renders the receipt
                    ...
                    {bookings.length === 0 ? (
                      ...
                      bookings.map((b) => (
                        <div key={b.key} className="flex items-center justify-between bg-white border border-stone-200 rounded-xl px-3 py-2 text-sm">
                          <div>
                            <p className="font-semibold text-stone-800">{sectionLabel(b.section)} Â· {b.customers.join(", ") || "-"}</p>
