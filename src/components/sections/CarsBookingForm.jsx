  ...
  <div className="bg-white border border-stone-200 rounded-2xl p-5 mb-6">
      ...
      <div>
        ...
        {carSupplierOther ? (
          ...
          <select
            ...
            value={carForm.supplier}
            onChange={(e) => {
              if (e.target.value === "__other__") { setCarSupplierOther(true); setCarForm({ ...carForm, supplier: "" }); }
              else setCarForm({ ...carForm, supplier: e.target.value });
            }}
          >
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
      <div>
        <label className="flex items-center gap-2 text-xs text-stone-500 mb-1 cursor-pointer">
          <input type="checkbox" checked={carForm.hasWaiting} onChange={(e) => setCarForm({ ...carForm, hasWaiting: e.target.checked, waitingHours: e.target.checked ? carForm.waitingHours : "" })} className="rounded border-stone-300" />
          Waiting hours
        </label>
      ...
      <div>
        <label className="flex items-center gap-2 text-xs text-stone-500 mb-1 cursor-pointer">
          <input type="checkbox" checked={carForm.isRoundTrip} onChange={(e) => setCarForm({ ...carForm, isRoundTrip: e.target.checked, returnDate: e.target.checked ? carForm.returnDate : "", returnTime: e.target.checked ? carForm.returnTime : "" })} className="rounded border-stone-300" />
      ...
      <div>
        <label className="flex items-center gap-2 text-xs text-stone-500 mb-1 cursor-pointer">
          <input type="checkbox" checked={carForm.startsAtAirport} onChange={(e) => setCarForm({ ...carForm, startsAtAirport: e.target.checked, flightNumber: e.target.checked ? carForm.flightNumber : "" })} className="rounded border-stone-300" />
    ...
    {carForm.isRoundTrip && (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="date" className="w-full border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700" value={carForm.returnDate} onChange={(e) => setCarForm({ ...carForm, returnDate: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-stone-500 block mb-1">Return time</label>
          <TimeSelect value={carForm.returnTime} onChange={(v) => setCarForm({ ...carForm, returnTime: v })} />
        </div>
      </div>
    )}
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
      <div>
        ...
        <select className="w-full border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 bg-white" value={carForm.currency} onChange={(e) => setCarForm({ ...carForm, currency: e.target.value })}>
          {HOTEL_CURRENCIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
        </select>
      </div>
      <div>
        <label className="text-xs text-stone-500 block mb-1">Collection</label>
        <div className="relative"><input type="number" className="w-28 border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 price-input" value={carForm.collection} onChange={(e) => setCarForm({ ...carForm, collection: e.target.value })} onBlur={(e) => setCarForm({ ...carForm, collection: addCentsOnBlur(e.target.value) })} placeholder="0" />{usdHint(carForm.collection, carForm.currency, carForm.usdRate) && <span className="absolute right-1 top-1/2 -translate-y-1/2 text-[8px] leading-none text-emerald-600 bg-white/90 pl-1 pointer-events-none truncate max-w-[70px]">{usdHint(carForm.collection, carForm.currency, carForm.usdRate)}</span>}</div>
      </div>
      <div>
        <label className="text-xs text-stone-500 block mb-1">Driver tip</label>
        <div className="relative"><input type="number" className="w-28 border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 price-input" value={carForm.driverTip} onChange={(e) => setCarForm({ ...carForm, driverTip: e.target.value })} onBlur={(e) => setCarForm({ ...carForm, driverTip: addCentsOnBlur(e.target.value) })} placeholder="0" />{usdHint(carForm.driverTip, carForm.currency, carForm.usdRate) && <span className="absolute right-1 top-1/2 -translate-y-1/2 text-[8px] leading-none text-emerald-600 bg-white/90 pl-1 pointer-events-none truncate max-w-[70px]">{usdHint(carForm.driverTip, carForm.currency, carForm.usdRate)}</span>}</div>
      </div>
      <div />
      <div>
        <label className="text-xs text-stone-500 block mb-1">Net currency</label>
        <select className="w-full border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 bg-white" value={carForm.netCurrency} onChange={(e) => setCarForm({ ...carForm, netCurrency: e.target.value })}>{HOTEL_CURRENCIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}</select>
      </div>
      <div>
        <label className="text-xs text-stone-500 block mb-1">Price net</label>
        <div className="relative"><input type="number" className="w-28 border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 price-input" value={carForm.netPrice} onChange={(e) => setCarForm({ ...carForm, netPrice: e.target.value })} onBlur={(e) => setCarForm({ ...carForm, netPrice: addCentsOnBlur(e.target.value) })} placeholder="0" />{usdHint(carForm.netPrice, carForm.netCurrency, carForm.usdRate) && <span className="absolute right-1 top-1/2 -translate-y-1/2 text-[8px] leading-none text-emerald-600 bg-white/90 pl-1 pointer-events-none truncate max-w-[70px]">{usdHint(carForm.netPrice, carForm.netCurrency, carForm.usdRate)}</span>}</div>
      </div>
      <div>
        <label className="text-xs text-stone-500 block mb-1">Sold currency</label>
        <select className="w-full border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 bg-white" value={carForm.soldCurrency} onChange={(e) => setCarForm({ ...carForm, soldCurrency: e.target.value })}>{HOTEL_CURRENCIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}</select>
      </div>
      <div>
        <label className="text-xs text-stone-500 block mb-1">Sold</label>
        <div className="relative"><input type="number" className="w-28 border border-stone-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700 price-input" value={carForm.soldPrice} onChange={(e) => setCarForm({ ...carForm, soldPrice: e.target.value })} onBlur={(e) => setCarForm({ ...carForm, soldPrice: addCentsOnBlur(e.target.value) })} placeholder="0" />{usdHint(carForm.soldPrice, carForm.soldCurrency, carForm.usdRate) && <span className="absolute right-1 top-1/2 -translate-y-1/2 text-[8px] leading-none text-emerald-600 bg-white/90 pl-1 pointer-events-none truncate max-w-[70px]">{usdHint(carForm.soldPrice, carForm.soldCurrency, carForm.usdRate)}</span>}</div>
      </div>
    </div>
    ...
    <div className="flex items-center gap-2">
      <button onClick={handleSaveCar} className="bg-gradient-to-b from-teal-700 to-teal-900 text-white text-sm font-semibold rounded-xl px-4 py-2 hover:brightness-110">
        {carEditingId ? "Save changes" : "Add transfer booking"}
      </button>
      {carEditingId && <button onClick={resetCarForm} className="text-sm text-stone-500 hover:text-stone-700 border border-stone-300 rounded-xl px-4 py-2">Cancel</button>}
    </div>
  </div>
);

export default CarsBookingForm;
