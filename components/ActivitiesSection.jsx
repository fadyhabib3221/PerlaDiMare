...
export default React.memo(function ActivitiesSection() {
  ...
  const searchActivityCities = async (rawQuery) => {
  ...
  return (
    <>
      {activeActivityWidgetId ? (() => {
        const widget = ACTIVITY_WIDGETS.find((item) => item.id === activeActivityWidgetId);
        if (!widget) return null;
        return <div className="bg-white rounded-2xl border border-stone-200 p-4 mb-4"><button onClick={() => setActiveActivityWidgetId(null)} className="flex items-center gap-1.5 text-xs font-semibold text-stone-500 hover:text-stone-700 mb-3"><ArrowLeft size={14} /> Back</button><h2 className="font-semibold text-stone-900 text-sm mb-3">{widget.title}</h2><TravelpayoutsWidget src={widget.src} /></div>;
      })() : <div className="bg-white rounded-2xl border border-stone-200 p-4 mb-4"><h2 className="font-semibold text-stone-900 text-sm mb-3">Quick search</h2><div className="flex flex-wrap gap-4">{ACTIVITY_WIDGETS.map((widget) => { const Icon = widget.icon; return <button key={widget.id} onClick={() => setActiveActivityWidgetId(widget.id)} className="flex flex-col items-center gap-1.5 w-20 group"><span className="w-14 h-14 rounded-2xl bg-teal-50 text-teal-800 flex items-center justify-center group-hover:bg-teal-100 transition-colors"><Icon size={22} /></span><span className="text-[11px] font-medium text-stone-600 text-center leading-tight">{widget.title}</span></button>; })}</div></div>}
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-4 mb-4"><h2 className="font-semibold text-stone-900 text-sm mb-1">Activities & tours</h2><p className="text-xs text-stone-500 mb-3">Search a destination city to see bookable tours, tickets, and audio guides. Every "Book / copy link" button gives you a ready booking link — bookings made through it are tracked to this account.</p><div className="relative max-w-md"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" /><input value={activityCityQuery} onChange={(e) => { const value = e.target.value; setActivityCityQuery(value); searchActivityCities(value); }} placeholder="Search a city (e.g. Paris, Rome, Cairo)…" className="w-full border border-stone-300 rounded-xl pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700" /></div>{activityCitySearching && <p className="text-xs text-stone-400 mt-2">Searching…</p>}{activityCityError && <p className="text-xs text-red-500 mt-2">{activityCityError}</p>}{activityCityResults.length > 0 && <div className="flex flex-wrap gap-1.5 mt-3">{activityCityResults.slice(0, 12).map((city) => <button key={city.id} onClick={() => loadActivityProducts(city)} className={`flex items-center gap-1.5 text-xs font-medium rounded-full px-3 py-1.5 border transition-colors ${activitySelectedCity && activitySelectedCity.id === city.id ? "bg-teal-700 border-teal-700 text-white" : "bg-white border-stone-300 text-stone-600 hover:bg-stone-50"}`}><MapPin size={12} /> {city.name || city.title}</button>)}</div>}</div>

      {activitySelectedCity && <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden"><div className="px-4 py-3 border-b border-stone-100 flex items-center justify-between"><h2 className="font-semibold text-stone-900 text-sm">Activities in {activitySelectedCity.name || activitySelectedCity.title}</h2>{activityProductsLoading && <span className="text-xs text-stone-400">Loading…</span>}</div>{activityProductsError && <p className="text-xs text-red-500 px-4 py-3">{activityProductsError}</p>}<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">{activityProducts.map((product) => <div key={product.id} className="border border-stone-200 rounded-xl overflow-hidden flex flex-col">{product.preview && <img src={product.preview} alt={product.title} className="w-full h-32 object-cover" loading="lazy" />}<div className="p-3 flex flex-col gap-1.5 flex-1"><p className="text-sm font-semibold text-stone-800 leading-snug">{product.title}</p><p className="text-xs text-stone-500">{product.category}{product.duration ? ` · ${product.duration}` : ""}</p>{!!product.rating && <p className="text-xs text-amber-600 font-medium">★ {product.rating} ({product.reviewsCount || 0})</p>}<div className="mt-auto flex items-center justify-between pt-2"><span className="text-sm font-bold text-stone-800">{product.currency}{product.price}</span><div className="flex items-center gap-1.5"><a href={activityProductLink(product)} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-white bg-teal-800 rounded-lg px-2.5 py-1.5 hover:bg-teal-900">Book</a><button onClick={() => navigator.clipboard && navigator.clipboard.writeText(activityProductLink(product))} title="Copy booking link" className="text-xs font-semibold text-stone-500 border border-stone-300 rounded-lg px-2 py-1.5 hover:bg-stone-50"><Copy size={13} /></button></div></div></div></div>)}</div></div>}
    </>
  );
});
