...
export default function TimeSelect({ value, onChange }) {
  ...
  return (
    <div className="w-full flex items-center border border-stone-300 rounded-xl px-3 py-2 text-sm focus-within:outline-none focus-within:ring-2 focus-within:ring-teal-700 bg-white">
      <select aria-label="Hour" className="flex-1 bg-transparent focus:outline-none appearance-none text-center" value={hour} onChange={(event) => update(event.target.value, minute)}>
        <option value="">--</option>
        {TIME_HOURS.map((item) => <option key={item} value={item}>{item}</option>)}
      </select>
      <span className="text-stone-400 px-0.5">:</span>
      <select aria-label="Minute" className="flex-1 bg-transparent focus:outline-none appearance-none text-center" value={minute} onChange={(event) => update(hour, event.target.value)}>
        <option value="">--</option>
        {TIME_MINUTES.map((item) => <option key={item} value={item}>{item}</option>)}
      </select>
    </div>
  );
}
